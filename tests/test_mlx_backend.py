...
class TestMLXDetection:
    def test_detect_mlx_not_installed(self, monkeypatch):
        ...
        from souplite.utils import mlx as mlx_utils
    ...
    def test_estimate_mlx_batch_size_small_model(self):
        from souplite.utils.mlx import estimate_mlx_batch_size
...
class TestMLXOtherTrainers:
    def test_mlx_dpo_import(self):
        from souplite.trainer.mlx_dpo import MLXDPOTrainerWrapper

        assert MLXDPOTrainerWrapper is not None

    def test_mlx_grpo_import(self):
        from souplite.trainer.mlx_grpo import MLXGRPOTrainerWrapper
...
class TestMLXRouting:
    def test_mlx_routing_map(self):
        ...
        from souplite.trainer import mlx_routing
...
class TestMLXDoctor:
    ...
    def test_doctor_command_mlx_absent_control(self, monkeypatch):
        ...
        monkeypatch.setattr("souplite.utils.mlx.is_apple_silicon", lambda: True)
        result = CliRunner().invoke(app, ["doctor"])

        assert result.exit_code == 0
        assert "MLX" in result.output
        assert 'pip install "souplite[mlx]"' in strip_ansi(result.output)
    ...
    def test_doctor_command_omits_mlx_panel_off_apple_silicon(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "souplite.commands.doctor._get_mlx_info",
        ...
        assert 'pip install "souplite[mlx]"' not in result.output
