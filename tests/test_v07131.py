...
def _trl_has_judges():
    ...
    from souplite.trainer.online_dpo import _trl_has_judges as has_judges
...
class _FakeJudge:
    ...
    def evaluate(self, prompt, response, category="default"):
        from souplite.eval.judge import JudgeScore
...
class TestPairwiseCompare:
    def test_a_preferred(self):
        from souplite.eval.judge import pairwise_compare
...
class TestPairwiseWinrate:
    def test_tuned_always_wins(self):
        from souplite.eval.judge import pairwise_winrate
...
class TestParsePairwise:
    def test_json_winner_a(self):
        from souplite.eval.judge import _parse_pairwise
...
class TestSoupPairwiseJudge:
    def test_judge_returns_best_index(self):
        from souplite.eval.judge import make_soup_pairwise_judge
    ...
    def test_is_trl_base_pairwise_judge(self):
        from souplite.eval.judge import (
...
class TestJudgeRewardFunc:
    def test_named_and_scores_by_evaluate(self):
        from souplite.eval.judge import make_judge_reward_func
...
class TestShipPairwise:
    def test_pairwise_in_supported(self):
        from souplite.utils.ship_verdict import SUPPORTED_TASK_MODES

        assert "pairwise" in SUPPORTED_TASK_MODES

    def test_leg1_pairwise_builds_taskwin(self, monkeypatch):
        from souplite.commands import ship as ship_cmd

        monkeypatch.setattr(
            "souplite.eval.custom.load_eval_tasks",
            lambda path: [_Task("q1"), _Task("q2")],
        )
        monkeypatch.setattr(
            "souplite.eval.gate._parse_judge_url",
            lambda url: ("ollama", "m", None),
        )
        monkeypatch.setattr(
            "souplite.eval.judge.JudgeEvaluator", lambda **kw: _FakeJudge()
        )
    ...
    def test_evidence_pairwise_accepted(self):
        ...
        from souplite.commands.ship import app
...
class TestOnlineDpoWrapper:
    ...
    def test_setup_builds_trainer_with_synthetic_judge(self):
        ...
        import souplite.trainer.online_dpo as od
        from souplite.config.loader import load_config_from_string
        from souplite.trainer.online_dpo import OnlineDPOTrainerWrapper
...
class TestBuildJudgeOrReward:
    def test_judge_url_branch_adapts_to_trl_version(self):
        ...
        if _TRL_HAS_JUDGES:  # trl <1 -> pairwise judge=
            from souplite.eval.judge import _base_pairwise_judge_cls
    ...
    def test_reward_model_branch_adapts_to_trl_version(self, monkeypatch):
        ...
        from souplite.trainer.online_dpo import _trl_accepts
...
class TestBestOfN:
    def test_provider_generate_fn_is_called_n_times(self):
        from souplite.utils.best_of_n import sample_candidates
    ...
    def test_pick_best_argmax(self):
        from souplite.utils.best_of_n import judge_pick_best
    ...
    def test_build_sft_row(self):
        from souplite.utils.best_of_n import BestOfNPick, build_sft_row
    ...
    def test_build_dpo_pair(self):
        from souplite.utils.best_of_n import BestOfNPick, build_dpo_pair
    ...
    def test_three_candidates_non_boundary_winner_and_loser(self):
        from souplite.utils.best_of_n import build_dpo_pair, judge_pick_best
    ...
    def test_no_top_level_torch(self):
        ...
        src = (
            pathlib.Path(souplite.__file__).parent / "utils" / "best_of_n.py"
...
class TestBestOfNCli:
    ...
    def _invoke_guard(monkeypatch, tmp_path, *sampler_args):
        ...
        from souplite.commands.data import app

        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr("souplite.eval.judge.JudgeEvaluator", lambda **kw: _ScoreJudge())
    ...
    def test_provider_path_calls_sampler_and_records_provenance(self, monkeypatch):
        ...
        import souplite.commands.data as data_cmd
        from souplite.commands.data import app
        ...
        monkeypatch.setattr("souplite.utils.magpie.make_magpie_generate_fn", fake_make)
    ...
    def test_happy_path(self, monkeypatch):
        ...
        import souplite.commands.data as data_cmd
        import souplite.utils.best_of_n as bon
        from souplite.commands.data import app
        ...
        monkeypatch.setattr(
            "souplite.utils.magpie.make_magpie_generate_fn",
    ...
    def test_non_finite_judge_score_does_not_publish_outputs(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "souplite.eval.judge.JudgeEvaluator", lambda **kwargs: _NonFiniteJudge()
        )
        prompts = tmp_path / "prompts.jsonl"
    ...
    def test_blank_lines_are_ignored_but_source_lines_remain_physical(
        ...
        from souplite.commands.data import _bon_load_prompt_records
...
class TestEvolve:
    def test_depth_round_produces_lineage(self):
        from souplite.utils.evolve import run_evolve
    ...
    def test_evolve_instruction_renders_seed(self):
        from souplite.utils.evolve import evolve_instruction
    ...
    def test_no_top_level_torch(self):
        ...
        tree = ast.parse(
            (pathlib.Path(souplite.__file__).parent / "utils" / "evolve.py").read_text(
                encoding="utf-8"
            )
        )
        top = {
...
class TestEvolveCli:
    ...
    def test_happy_path(self, monkeypatch):
        ...
        monkeypatch.setattr("souplite.utils.magpie.make_magpie_generate_fn", _fake_make)
