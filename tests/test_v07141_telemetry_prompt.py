...
def test_emit_telemetry_disabled_never_builds_or_sends(monkeypatch):
    ...
    from souplite.cli import _emit_telemetry

    monkeypatch.delenv("SOUP_TELEMETRY", raising=False)
    build_mock = MagicMock()
    send_mock = MagicMock()
    monkeypatch.setattr("souplite.utils.trackers.build_telemetry_payload", build_mock)
    monkeypatch.setattr("souplite.utils.trackers.send_telemetry_payload", send_mock)
    monkeypatch.setattr(
...
def test_emit_telemetry_enabled_builds_and_sends(tmp_path, monkeypatch):
    ...
    sent_payloads = []
    monkeypatch.setattr(
        "souplite.utils.trackers.send_telemetry_payload",
...
def test_distinct_id_creation_persistence_and_reuse(tmp_path, monkeypatch):
    ...
    from souplite.utils.trackers import get_or_create_distinct_id
...
def test_build_telemetry_payload_disabled_leaves_no_disk_trace(tmp_path, monkeypatch):
    ...
    from souplite.utils.trackers import build_telemetry_payload
...
def test_emit_telemetry_never_raises_on_broken_env(monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.utils.trackers.is_telemetry_enabled",
