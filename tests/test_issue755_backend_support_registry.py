...
def test_an_entry_naming_a_field_that_no_longer_exists_is_caught():
    ...
    """
    from souplite.config.backend_support import IGNORED, SupportEntry
...
def test_every_registry_entry_carries_a_status_and_a_reason():
    from souplite.config.backend_support import REGISTRY, STATUSES
...
def test_registry_covers_every_field_the_mlx_trainer_warns_about():
    ...
    """
    from souplite.config.backend_support import unsupported_for
...
def test_warning_guard_preserves_digits_and_catches_missing_entries(tmp_path, monkeypatch, field):
    from souplite.config import backend_support
    from souplite.config.backend_support import IGNORED, SupportEntry
...
def test_doctor_config_names_the_ignored_field_and_its_reason(
    ...
    import souplite.commands.doctor as doctor_module
    from souplite.commands.doctor import doctor
...
def _registry_drift(repo_root: pathlib.Path) -> list[str]:
    ...
    """
    from souplite.config.backend_support import REGISTRY, TRAINER_MODULES
...
def test_the_declared_modules_cover_every_helper_the_trainer_imports():
    ...
    """
    from souplite.config.backend_support import TRAINER_MODULES
    ...
    for pair, modules in TRAINER_MODULES.items():
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom) and node.module:
                if node.module.startswith("souplite.trainer"):
                    imported.add(node.module)
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.startswith("souplite.trainer"):
...
def test_an_unfounded_gap_claim_is_caught():
    """@MuhtarJaksilikov's mutation on #756, which the first version survived.
    ...
    """
    import souplite.config.backend_support as bs
...
def test_a_declared_module_that_does_not_exist_is_caught(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(
        bs.TRAINER_MODULES, ("sft", "mlx"), ("souplite/trainer/gone.py",)
    )
...
def test_a_field_wired_only_inside_a_helper_is_caught(tmp_path, monkeypatch):
    ...
    trainer = tmp_path / "src" / "souplite" / "trainer"
    ...
    monkeypatch.setitem(
        bs.TRAINER_MODULES,
        ("sft", "mlx"),
        ("souplite/trainer/mlx_sft.py", "souplite/trainer/mlx_optim.py"),
    )
...
def test_wiring_a_field_makes_its_stale_entry_fail(tmp_path, monkeypatch):
    ...
    fake_src = tmp_path / "src" / "souplite" / "trainer"
    ...
    monkeypatch.setitem(
        bs.TRAINER_MODULES, ("sft", "mlx"), ("souplite/trainer/mlx_sft.py",)
    )
