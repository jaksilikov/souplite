...
from souplite.utils.canary_router import (
    BucketStats,
    CanaryPolicy,
    rollback,
    route,
)
from souplite.utils.loop_budget import (
    check_budget,
    parse_budget_string,
    reset_daily_counter_if_new_day,
)
from souplite.utils.loop_daemon import (
    WatchConfig,
...
from souplite.utils.loop_iteration import (
    IterationRecord,
    list_iterations,
    new_iteration_id,
    read_iteration,
...
from souplite.utils.loop_state import (
    LOOP_STATUSES,
    LoopState,
    default_state_path,
    init_state,
...
class TestSourceWiring:
    def test_cli_registers_loop_typer(self):
        cli_src = (_REPO_ROOT / "src" / "souplite" / "cli.py").read_text(encoding="utf-8")
        assert "from souplite.commands import loop as _loop_cmd" in cli_src
    ...
    def test_version_bumped_to_0_58_0(self):
        ...
        init = (_REPO_ROOT / "src" / "souplite" / "__init__.py").read_text(encoding="utf-8")
    ...
    def test_no_top_level_torch_import_in_loop_modules(self):
        for name in [
            ...
            src = (_REPO_ROOT / "src" / "souplite" / "utils" / name).read_text(encoding="utf-8")
    ...
    def test_command_module_uses_typer_app(self):
        src = (_REPO_ROOT / "src" / "souplite" / "commands" / "loop.py").read_text(encoding="utf-8")
