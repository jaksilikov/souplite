...
from souplite.data.loss_mask import IGNORE_INDEX, coerce_token_ids
...
class TestPublicSharedHelper:
    ...
    def test_data_doctor_imports_the_loss_mask_helper_not_a_copy(self):
        ...
        import souplite.utils.data_doctor as doctor
        ...
        for node in ast.walk(tree):
            if (
                ...
                and node.module == "souplite.data.loss_mask"
            ):
    ...
    def test_no_module_imports_loss_mask_private_coerce(self):
        src_root = Path(__file__).resolve().parents[1] / "src" / "souplite"
        offenders: list[str] = []
        for path in src_root.rglob("*.py"):
            if path.name == "loss_mask.py":
                continue
            text = path.read_text(encoding="utf-8")
            if "from souplite.data.loss_mask import" in text and "_coerce_token_ids" in text:
...
class TestOneMappingPredicate:
    def test_both_call_sites_share_the_mapping_predicate(self):
        ...
        import souplite.data.loss_mask as mod
