...
class TestReadPrompts:
    ...
    def test_read_jsonl_prompts(self, tmp_path):
        ...
        from souplite.commands.infer import _read_prompts
...
class TestLoadModel:
    ...
    def test_adapter_without_base_model_exits(self, tmp_path):
        ...
        from souplite.commands.infer import _load_model
...
class TestGenerate:
    ...
    def test_generate_with_chat_template(self):
        ...
        from souplite.commands.infer import _generate
...
class TestInferImports:
    ...
    def test_import_infer_module(self):
        from souplite.commands.infer import infer
...
# ─── Issue #1097: HF Repo ID Preservation Tests ─────────────────────────────


class TestInferHFRepoId:
    """Ensure HuggingFace repo IDs are not corrupted with OS-specific path separators (#1097)."""

    def test_cli_passes_hf_repo_id_byte_identical_with_slashes(self, tmp_path, monkeypatch):
        """soup infer --model owner/repo must pass repo ID with forward slashes (#1097)."""
        ...
        runner = CliRunner()
        with patch("souplite.commands.infer._load_model", mock_load), \
             patch("souplite.commands.infer._generate", return_value=("answer", 5)):
            result = runner.invoke(app, [
                "infer",
        ...
        passed_model_path = mock_load.call_args[0][0]
    ...
    def test_load_model_hf_repo_id_passes_slash_to_transformers(self):
        ...
        with patch(
            ...
            "souplite.utils.trust_remote.resolve_trust_remote_code", return_value=False
        ):
            _load_model(repo_id, None, "cpu", trust_remote_code=False)

            mock_tok.assert_called_once()
            called_tok_arg = mock_tok.call_args[0][0]
            assert called_tok_arg == repo_id
            assert "\\" not in called_tok_arg
            assert "/" in called_tok_arg

            mock_model.assert_called_once()
            called_model_arg = mock_model.call_args[0][0]
