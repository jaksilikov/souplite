...
_RUNTIME_FLOOR_REASON = (
    "requires the validated training floor; install souplite[train] with "
...
def test_doctor_training_bounds_match_declared_extra():
    from souplite.commands.doctor import _MAX_EXCLUSIVE, EXTRA_GROUPS
...
def test_qwen35_auto_targets_attach_and_reload_a_nonempty_adapter(tmp_path):
    ...
    from souplite.utils.peft_wiring import resolve_lora_target_modules
...
def test_explicit_peft_targets_and_mlx_defaults_are_unchanged():
    from souplite.trainer.mlx_sft import MLX_DEFAULT_TARGET_KEYS
    from souplite.utils.peft_wiring import resolve_lora_target_modules
