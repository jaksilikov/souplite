...
def _export_candidates(tmp_path, monkeypatch, prompts=("question one", "question two")):
    ...
    monkeypatch.setattr("souplite.utils.magpie.make_magpie_generate_fn", factory)
    monkeypatch.setattr(
...
def test_offline_materialization_never_constructs_sampler_or_judge_and_is_stable(
    ...
    monkeypatch.setattr("souplite.commands.data._load_bon_model", forbidden)
    monkeypatch.setattr("souplite.eval.judge.JudgeEvaluator", forbidden)
    monkeypatch.setattr("souplite.utils.magpie.make_magpie_generate_fn", forbidden)
...
def test_offline_manifest_commits_and_verifies_exact_output_set(tmp_path, monkeypatch):
    from souplite.commands.data import app
    from souplite.utils.best_of_n_artifact import verify_offline_manifest
...
def test_two_phase_artifacts_use_exact_utf8_byte_writes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.utils.paths.atomic_write_text",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
