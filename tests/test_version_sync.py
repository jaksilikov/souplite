...
`pyproject.toml` and `src/souplite/__init__.py` both carry the version, and the
...
INIT = ROOT / "src" / "souplite" / "__init__.py"
...
def _init_version(text: str) -> str:
    ...
    assert match, "no __version__ line in src/souplite/__init__.py"
    return match.group(1)
...
class TestVersionSync:
    def test_the_two_files_agree(self):
        ...
        assert declared == package, (
            f"pyproject.toml says {declared!r} but src/souplite/__init__.py says "
