...
def test_module_imports():
    from souplite.utils import env_lock
...
def test_tracked_packages_contains_core():
    from souplite.utils.env_lock import TRACKED_PACKAGES
...
def test_env_entry_frozen():
    from souplite.utils.env_lock import EnvEntry
...
def test_env_lock_frozen():
    from souplite.utils.env_lock import EnvEntry, EnvLock
...
def test_env_lock_rejects_null_byte_in_platform():
    from souplite.utils.env_lock import EnvLock
...
def test_snapshot_env_returns_envlock():
    from souplite.utils.env_lock import EnvLock, snapshot_env
...
def test_snapshot_env_entries_non_empty():
    ...
    from souplite.utils.env_lock import snapshot_env
...
def test_write_lock_roundtrip(tmp_path, monkeypatch):
    from souplite.utils.env_lock import EnvEntry, EnvLock, read_lock, write_lock
...
def test_write_lock_outside_cwd_rejected(tmp_path, monkeypatch):
    from souplite.utils.env_lock import EnvLock, write_lock
...
def test_read_lock_missing(tmp_path, monkeypatch):
    from souplite.utils.env_lock import read_lock
...
def test_abi_check_frozen():
    from souplite.utils.env_lock import AbiCheck
...
def test_check_abi_compat_no_drift():
    from souplite.utils.env_lock import EnvEntry, EnvLock, check_abi_compat
...
def test_check_abi_compat_cuda_change():
    from souplite.utils.env_lock import EnvLock, check_abi_compat
...
def test_check_abi_compat_rejects_non_envlock():
    from souplite.utils.env_lock import check_abi_compat
...
def test_cli_registers_env():
    ...
    src = Path(__file__).resolve().parent.parent / "src" / "souplite" / "cli.py"
...
def test_no_heavy_top_level_imports():
    ...
    src = Path(__file__).resolve().parent.parent / "src" / "souplite" / "utils" / "env_lock.py"
    text = src.read_text(encoding="utf-8")
    import re
...
def test_compute_env_hash_is_64_hex():
    ...
    from souplite.utils.env_lock import compute_env_hash
...
def test_compute_env_hash_matches_lock_closure_regex():
    ...
    from souplite.utils.env_lock import compute_env_hash
    from souplite.utils.soup_lock import compute_lock_closure
...
def test_render_install_plan_uv_format():
    from souplite.utils.env_lock import render_install_plan
...
def test_write_requirements_txt_round_trip(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    from souplite.utils.env_lock import write_requirements_txt
...
def test_cli_env_check_drift_exits_3(tmp_path, monkeypatch):
    from souplite.cli import app
    from souplite.utils.env_lock import snapshot_env, write_lock
...
# ---------------------------------------------------------------------------
# check_declared_bounds — an installed package that violates souplite's OWN
...
# like: since the v0.71.0 deps-split, `importlib.metadata.requires("souplite")`
...
def test_check_declared_bounds_flags_core_bound_over_cap():
    from souplite.utils.env_lock import check_declared_bounds
...
def test_extra_gated_untracked_bound_stays_deselected():
    ...
    from souplite.utils.env_lock import TRACKED_PACKAGES, check_declared_bounds
...
def test_declared_bounds_warns_when_packaging_missing(caplog, monkeypatch):
    ...
    with caplog.at_level(logging.WARNING, logger="souplite.utils.env_lock"):
...
def test_current_declared_bounds_check_returns_bounds_check_for_absent_dist():
    from souplite.utils.env_lock import BoundsCheck, current_declared_bounds_check
...
def test_current_declared_bounds_check_real_path_detects_violation(monkeypatch):
    ...
    report = env_lock.current_declared_bounds_check("souplite")
    assert not report.ok
    assert report.violation_count == 1
    assert report.violations[0].name == "transformers"
...
def test_cli_env_check_declared_bound_violation_exits_3(tmp_path, monkeypatch):
    from souplite.cli import app
    from souplite.commands import env as env_cmd
    from souplite.utils.env_lock import BoundsCheck, BoundViolation
