...
SCHEMA_PATH = ROOT / "src" / "souplite" / "config" / "schema.py"
COMMAND_PATHS = (
    ROOT / "src" / "souplite" / "cli.py",
    *(ROOT / "src" / "souplite" / "commands").glob("*.py"),
)
...
def test_named_docs_no_longer_promise_past_follow_up_releases() -> None:
    stale_fragments = {
        "docs/data.md": (
            ...
            "Live wiring of the proxy training loop into a short `soup train` run is the "
            "v0.48.1 deliverable",
        ),
