...
from souplite.utils.eval_lock_coverage import (
    compute_coverage,
    lock_suite,
)
...
class TestImmutableRegistries:
    def test_recommended_scorers_is_mappingproxy(self):
        from souplite.utils.eval_lock_coverage import _RECOMMENDED_SCORERS
    ...
    def test_metric_direction_is_mappingproxy(self):
        from souplite.utils.eval_gate_hook import _METRIC_DIRECTION
...
class TestShellEscape:
    ...
    def test_render_no_handrolled_escape_symbol_remains(self):
        from souplite.utils import eval_gate_hook as mod
...
class TestNoHeavyImports:
    ...
    @pytest.mark.parametrize(
        "module",
        [
            "src/souplite/utils/eval_design.py",
            "src/souplite/utils/canary_discovery.py",
            "src/souplite/utils/eval_lock_coverage.py",
            "src/souplite/utils/eval_gate_hook.py",
            "src/souplite/utils/_eval_text.py",
            "src/souplite/commands/_eval_v0550.py",
        ],
    )
...
class TestSharedTextUtils:
    def test_shared_module_exposes_row_text_and_tokenize(self):
        from souplite.utils import _eval_text
    ...
    def test_canary_no_longer_imports_from_eval_design(self):
        ...
        assert "from souplite.utils.eval_design import _row_text" not in src
        assert "from souplite.utils._eval_text import" in src
