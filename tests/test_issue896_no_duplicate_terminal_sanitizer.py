...
v0.75.0 added ``src/souplite/utils/terminal.py`` (``for_terminal``: strip C0/DEL
...
definer -- confirmed by ``grep -rn "range(0x20)" src/souplite``.
"""
...
SRC_ROOT = REPO_ROOT / "src" / "souplite"
...
def _offenders() -> list[str]:
    ...
    for path in _tracked_python_files():
        ...
        try:
            ...
            # An unparseable file under ``src/souplite`` is a worse problem
...
class TestNoDuplicateTerminalSanitizer:
    def test_no_module_besides_terminal_defines_the_strip_table(self):
        ...
        assert not offenders, (
            ...
            "instead of importing souplite.utils.terminal.for_terminal / "
            "strip_control (#896):\n  " + "\n  ".join(offenders)
        )
...
class TestTheScannerCanActuallyFail:
    ...
    def test_an_unparseable_file_in_the_tree_fails_the_guard(self):
        """A file under ``src/souplite`` that does not parse must fail this
