...
Tests for ``souplite/utils/cmaes_merge.py``:
...
class TestPublicSurface:
    def test_module_importable(self) -> None:
        from souplite.utils import cmaes_merge
...
class TestValidatePopulationSize:
    def test_happy(self) -> None:
        from souplite.utils.cmaes_merge import validate_population_size
...
class TestValidateGenerations:
    def test_happy(self) -> None:
        from souplite.utils.cmaes_merge import validate_generations
...
class TestCmaesPlan:
    def test_construct(self) -> None:
        from souplite.utils.cmaes_merge import CmaesPlan
...
class TestCmaesResult:
    def test_construct(self) -> None:
        from souplite.utils.cmaes_merge import CmaesResult
...
class TestRunCmaesMerge:
    ...
    def test_non_plan_rejected(self) -> None:
        from souplite.utils.cmaes_merge import run_cmaes_merge
...
class TestSourceWiring:
    def test_no_top_level_heavy_imports(self) -> None:
        ...
        src = (root / "src" / "souplite" / "utils" / "cmaes_merge.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_adapter_merge_lists_cmaes(self) -> None:
        ...
        root = Path(__file__).resolve().parent.parent
        src = (root / "src" / "souplite" / "utils" / "adapter_merge.py").read_text(
            encoding="utf-8"
        )
        assert "cmaes" in src
