...
def test_load_reward_fn_requires_domain_and_config_carries_it():
    ...
    from souplite.trainer.rewards import load_reward_fn, math_verify_reward
...
def test_ppo_setup_reward_loads_verifiable_end_to_end():
    ...
    from souplite.trainer.rewards import math_verify_reward
...
def test_grpo_setup_passes_verifiable_domain(monkeypatch):
    ...
    # grpo.setup() does `from souplite.trainer.rewards import load_reward_fn`
    # at call time, so patching the source module attribute is effective.
    monkeypatch.setattr("souplite.trainer.rewards.load_reward_fn", _spy)
...
def test_trainer_wires_eval_gate_config(name):
    ...
    src = (Path(souplite.__file__).parent / "trainer" / f"{name}.py").read_text(
        encoding="utf-8"
    )
