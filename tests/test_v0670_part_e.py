...
Tests for ``souplite/utils/soup_lock.py``:
...
class TestPublicSurface:
    def test_module_importable(self) -> None:
        from souplite.utils import soup_lock
...
class TestSoupLock:
    def test_construct(self) -> None:
        from souplite.utils.soup_lock import SoupLock
...
class TestWriteReadLock:
    def test_roundtrip(self, tmp_path, monkeypatch) -> None:
        from souplite.utils.soup_lock import (
            SoupLock,
            read_lock,
            write_lock,
        )
    ...
    def test_outside_cwd_rejected(self, tmp_path, monkeypatch) -> None:
        from souplite.utils.soup_lock import SoupLock, write_lock
    ...
    def test_read_missing(self, tmp_path, monkeypatch) -> None:
        from souplite.utils.soup_lock import read_lock
...
class TestCheckLockDrift:
    ...
    def test_no_drift(self) -> None:
        from souplite.utils.soup_lock import check_lock_drift
...
class TestCliSmoke:
    def test_lock_help(self) -> None:
        ...
        from souplite.commands.lock import app
    ...
    def test_lock_write_auto_derives_env_hash(self, tmp_path, monkeypatch) -> None:
        ...
        from souplite.commands.lock import app
        from souplite.utils.env_lock import compute_env_hash, snapshot_env, write_lock
...
class TestSourceWiring:
    def test_no_top_level_heavy_imports(self) -> None:
        ...
        src = (root / "src" / "souplite" / "utils" / "soup_lock.py").read_text(
            encoding="utf-8"
        )
        head_lines = [
            line
