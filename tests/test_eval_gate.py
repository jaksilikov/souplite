...
class TestRunGate:
    ...
    def test_run_gate_passing(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from souplite.eval.gate import EvalSuite, GateTask, run_gate
...
class TestCallbackIntegration:
    def test_callback_runs_gate_on_epoch_end(self, tmp_path):
        from souplite.eval.gate import EvalSuite, GateResult, GateTaskResult
    ...
    def test_callback_skips_when_not_epoch_boundary(self):
        ...
        from souplite.eval.gate import EvalSuite
        from souplite.monitoring.callback import SoupTrainerCallback
...
class TestJudgeModelValidation:
    def test_rejects_localhost_prefix_bypass(self):
        from souplite.eval.gate import GateTask
...
class TestJudgeTaskNormalization:
    ...
    def _run_with_mock_judge(self, prompts_file, overall_score, rubric=None):
        from souplite.eval.gate import GateTask, _run_judge_task
        from souplite.eval.judge import DEFAULT_RUBRIC, JudgeResults
        ...
        with pytest.MonkeyPatch.context() as mp:
            mp.setattr("souplite.eval.judge.JudgeEvaluator", lambda **kw: mock_evaluator)
    ...
    def test_end_to_end_gate_passes_for_perfect_judge_score(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("souplite.eval.judge.JudgeEvaluator", lambda **kw: mock_evaluator)
