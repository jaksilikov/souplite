...
def test_late_candidate_failure_resumes_without_replaying_completed_groups(
    ...
    from souplite.commands.data import app
    from souplite.utils.best_of_n_artifact import load_candidate_artifact
    ...
    monkeypatch.setattr("souplite.utils.magpie.make_magpie_generate_fn", failing_factory)
    ...
    monkeypatch.setattr("souplite.utils.magpie.make_magpie_generate_fn", resumed_factory)
...
def test_offline_large_artifacts_bypass_whole_file_materializers(tmp_path, monkeypatch):
    from souplite.commands.data import app
    from souplite.utils.best_of_n_artifact import (
    ...
    monkeypatch.setattr("souplite.utils.best_of_n_artifact.load_candidate_artifact", forbidden)
    monkeypatch.setattr("souplite.utils.best_of_n_artifact.load_judgments", forbidden)
    monkeypatch.setattr("souplite.utils.best_of_n_artifact.materialize_rows", forbidden)
    monkeypatch.setattr("souplite.utils.best_of_n_artifact.stable_jsonl", forbidden)
    from souplite.utils.best_of_n_stream import OfflineArtifactIndex
...
def test_resume_discards_an_uncommitted_candidate_tail(tmp_path, monkeypatch):
    from souplite.utils.best_of_n_artifact import build_candidate_group
    from souplite.utils.best_of_n_stream import (
...
def test_checkpoint_identity_is_private_and_not_published(tmp_path, monkeypatch):
    from souplite.commands.data import app
    from souplite.utils.best_of_n_artifact import sampler_identity_fingerprint
...
def test_resume_rejects_a_different_private_local_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.commands.data._load_bon_model", lambda *_args, **_kwargs: (object(), object())
    )
    monkeypatch.setattr(
        "souplite.utils.best_of_n.sample_candidates",
...
def test_resume_rejects_replaced_content_at_the_same_local_model_path(
    ...
    monkeypatch.setattr(
        "souplite.commands.data._load_bon_model",
...
def test_local_resume_reuses_the_same_prompt_seed(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.utils.best_of_n.sample_candidates", sample_candidates
    )
    first = CliRunner().invoke(app, _local_args(tmp_path, model))
...
def test_streamed_publication_restores_changed_generation_on_dpo_failure(
    ...
    from souplite.utils.best_of_n_stream import StagedDatasets, publish_staged_datasets
...
def test_atomic_group_removal_validation_is_directly_covered(tmp_path, monkeypatch):
    from souplite.utils.paths import atomic_write_bytes_group
