...
from souplite.commands.doctor import _version_ok
...
def test_doctor_missing_train_extra_suggests_extra(monkeypatch):
    ...
    assert 'pip install "souplite[train]"' in out
...
def test_doctor_partial_train_extra_names_missing_members(monkeypatch):
    ...
    monkeypatch.setattr("souplite.commands.doctor._installed_version_str", _fake_version)
    monkeypatch.setattr("souplite.commands.doctor._nvidia_smi_cuda_version", lambda: None)
    result = runner.invoke(app, ["doctor"])
    assert result.exit_code == 0
    out = _strip_ansi(result.output)
...
def test_doctor_missing_train_extra_message_unchanged_when_none_installed(monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.commands.doctor._installed_version_str", lambda import_name, pkg_name: None
    )
    monkeypatch.setattr("souplite.commands.doctor._nvidia_smi_cuda_version", lambda: None)
    result = runner.invoke(app, ["doctor"])
    assert result.exit_code == 0
    out = _strip_ansi(result.output)
    assert 'Training stack not installed: pip install "souplite[train]"' in out
...
def test_doctor_missing_train_extra_with_unsupported_driver_falls_back_to_extra(monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.commands.doctor._nvidia_smi_cuda_version",
        lambda: (11, 7),
    )
    result = runner.invoke(app, ["doctor"])
...
def test_doctor_suggestion_is_colour_safe(monkeypatch):
    ...
    monkeypatch.setattr("souplite.commands.doctor.console", Console(force_terminal=True))
    for name in (
        "torch",
        "transformers",
        "peft",
    ...
    result = runner.invoke(app, ["doctor"])
    assert result.exit_code == 0
    assert 'pip install "souplite[train]"' in _strip_ansi(result.output)
...
def test_doctor_fix_all_line_keeps_extra_marker(monkeypatch):
    ...
    assert 'pip install "souplite[train]"' in after
...
def test_doctor_incompatible_train_member_is_reported(monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.commands.doctor.EXTRA_GROUPS",
        [("train", [("transformers", "transformers", "5.16.1")])],
    )
    monkeypatch.setattr(
        "souplite.commands.doctor._installed_version_str", lambda import_name, pkg_name: "6.1.0"
    )
    result = runner.invoke(app, ["doctor"])
    out = _strip_ansi(result.output)
...
def test_doctor_incompatible_core_dependency_exits_nonzero(monkeypatch):
    ...
    monkeypatch.setattr("souplite.commands.doctor.DEPS", [("typer", "typer", "0.1.0", True)])
    monkeypatch.setattr("souplite.commands.doctor._MAX_EXCLUSIVE", {"typer": "0.2.0"})
    result = runner.invoke(app, ["doctor"])
    assert result.exit_code == 1
    assert "INCOMPATIBLE" in _strip_ansi(result.output)
...
def test_doctor_outdated_train_member_exits_zero(monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.commands.doctor._installed_version_str", lambda import_name, pkg_name: "4.0.0"
    )
    result = runner.invoke(app, ["doctor"])
    assert result.exit_code == 0
...
def test_doctor_partial_train_group_in_range_exits_zero(monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.commands.doctor._installed_version_str",
...
def test_doctor_nvidia_train_suggestion_is_two_step(monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.commands.doctor._nvidia_smi_cuda_version", lambda: (13, 0)
    )
    ...
    assert '["souplite[train]" --index-url' not in out and '[train]" --index-url' not in out
...
def test_installed_extras_lists_train_and_mcp(monkeypatch):
    ...
    from souplite.cli import _installed_extras
...
def test_doctor_nccl_no_gpu():
    ...
    with (
        ...
        patch(
            "souplite.utils.topology.detect_topology",
