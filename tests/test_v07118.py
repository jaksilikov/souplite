...
class TestMiniLLMConfigOnPolicy:
    def test_on_policy_field_default(self):
        from souplite.utils.minillm import MiniLLMConfig
...
class TestMiniLLMRolloutLengthSchema:
    ...
    def test_distill_trainer_threads_explicit_length(self):
        ...
        src = (REPO_ROOT / "src" / "souplite" / "trainer" / "distill.py").read_text(
            encoding="utf-8"
        )
...
class TestMiniLLMOnPolicyRollout:
    def test_rollout_returns_loss_and_steps(self):
        ...
        from souplite.utils.minillm import MiniLLMConfig, minillm_on_policy_rollout
    ...
    def test_rollout_rejects_bad_config(self):
        ...
        from souplite.utils.minillm import minillm_on_policy_rollout
...
class TestDistillOnPolicyWiring:
    def test_distill_routes_on_policy(self):
        src = (REPO_ROOT / "src/souplite/trainer/distill.py").read_text(encoding="utf-8")
        assert "on_policy" in src
        assert "on_policy_term" in src

    def test_minillm_no_top_level_torch(self):
        src = (REPO_ROOT / "src/souplite/utils/minillm.py").read_text(encoding="utf-8")
...
class TestULDAlignedStrategy:
    def test_strategy_registered(self):
        from souplite.utils.uld import SUPPORTED_ULD_STRATEGIES, validate_uld_strategy
    ...
    def test_aligned_does_not_require_top_k(self):
        from souplite.utils.uld import ULDConfig
...
class TestAlignTokenSequences:
    def test_identical_tokenization(self):
        from souplite.utils.uld import align_token_sequences
    ...
    def test_char_cap_truncates_difflib_branch(self):
        from souplite.utils.uld import _MAX_ALIGN_TOKENS, align_token_sequences
...
class TestAggregateAlignedLogits:
    def test_mean_pool(self):
        ...
        from souplite.utils.uld import aggregate_aligned_logits
...
class TestUldAlignedLoss:
    def test_finite_and_differentiable(self):
        ...
        from souplite.utils.uld import ULDConfig, uld_aligned_loss
...
class TestDistillUldAlignedWiring:
    ...
    def test_uld_no_top_level_torch(self):
        src = (REPO_ROOT / "src/souplite/utils/uld.py").read_text(encoding="utf-8")
        assert "\nimport torch" not in src
        assert "\nfrom torch" not in src
...
class TestBuildEvalStub:
    def test_stub_is_python_and_b64_only(self):
        from souplite.utils.agent_sandbox import build_eval_stub
    ...
    def test_required_params_extracted(self):
        from souplite.utils.agent_sandbox import _required_path_params
...
class TestClassifySandboxOutcome:
    def test_timeout(self):
        from souplite.utils.agent_sandbox import classify_sandbox_outcome
...
class TestRunEvalInSandbox:
    ...
    def test_run_ok_stub(self):
        from souplite.utils.agent_sandbox import build_eval_stub, run_eval_in_sandbox
...
class TestScoreSandbox:
    def test_score_ok(self):
        from souplite.utils.agent_sandbox import score_sandbox
    ...
    def test_score_uses_override(self, monkeypatch):
        import souplite.utils.agent_sandbox as m
...
class TestAgentSandboxNoTopLevelHeavyImport:
    def test_no_top_level_torch(self):
        src = (REPO_ROOT / "src/souplite/utils/agent_sandbox.py").read_text(
            encoding="utf-8"
        )
        assert "\nimport torch" not in src
        # rewards.py is imported lazily inside run_eval_in_sandbox (not at top).
        assert "\nfrom souplite.trainer.rewards import" not in src
...
class TestValidateCloud:
    def test_modal(self):
        from souplite.cloud.modal import validate_cloud
...
class TestValidateGpu:
    ...
    def test_known(self, gpu):
        from souplite.cloud.modal import validate_gpu
...
class TestRenderModalStub:
    def test_structure(self):
        ...
        assert "souplite[train]==0.71.18" in stub
        assert "base64.b64decode" in stub
    ...
    def test_oversize_config_rejected(self):
        from souplite.cloud.modal import _MAX_CONFIG_BYTES, render_modal_stub
    ...
    def test_soup_version_length_capped(self):
        from souplite.cloud.modal import _MAX_VERSION_LEN, render_modal_stub
    ...
    def test_soup_version_pep440ish_accepted(self):
        ...
        assert "souplite[train]==0.71.18.dev0+gabc123" in stub
...
class TestPlanModalRun:
    def test_plan(self, tmp_path, monkeypatch):
        ...
        from souplite.cloud.modal import CloudPlan, plan_modal_run
...
class TestWriteStub:
    def test_writes_under_cwd(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "soup.yaml").write_text(_SOUP_YAML, encoding="utf-8")
        from souplite.cloud.modal import plan_modal_run, write_stub
...
class TestSubmitModalRun:
    def test_override_seam(self, monkeypatch):
        import souplite.cloud.modal as m
    ...
    def test_non_plan_rejected(self):
        from souplite.cloud.modal import submit_modal_run
...
class TestCloudNoTopLevelModal:
    def test_no_top_level_modal_import(self):
        src = (REPO_ROOT / "src/souplite/cloud/modal.py").read_text(encoding="utf-8")
