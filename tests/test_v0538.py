...
* #93 — package-data migration (souplite/data/_fixtures/)
...
class TestDetectSpaceSdk:
    def _import(self):
        from souplite.utils.hf_space import detect_space_sdk
...
class TestIsSupportedSpaceSdk:
    def test_known(self):
        from souplite.utils.hf_space import is_supported_space_sdk
...
class TestSendTelemetryPayload:
    def test_disabled_short_circuits(self, monkeypatch):
        ...
        from souplite.utils.trackers import send_telemetry_payload
    ...
    def test_happy_path_2xx(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "souplite.utils.trackers._telemetry_endpoint_is_safe", lambda _url: True
        )
...
class TestTrackerMissingDepMessage:
    ...
    def test_missing_mlflow_advisory(self, monkeypatch):
        ...
        assert "souplite[trackers]" in msg
...
class TestRemoteLoader:
    def test_looks_like_remote_uri(self):
        from souplite.data.loader import _looks_like_remote_uri
    ...
    def test_fsspec_missing_raises_friendly(self, monkeypatch, tmp_path):
        ...
        from souplite.data.loader import _load_remote_dataset
...
class TestPackageDataFixtures:
    def test_fixtures_dir_exists(self):
        ...
        pkg = Path(souplite.__file__).parent
    ...
    def test_bundle_source_prefers_package_data(self):
        from souplite.utils.demo_bundles import _bundle_source_path, get_bundle

        bundle = get_bundle("alpaca_demo")
        src = _bundle_source_path(bundle)
        # Should resolve under souplite/data/_fixtures (not examples/data/).
...
class TestVersionBump:
    ...
    def test_init_version(self):
        ...
        assert self._version_tuple(souplite.__version__) >= (0, 53, 8)
