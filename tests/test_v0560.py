...
from souplite.utils.diagnose import (
    FAILURE_MODES,
    FailureReport,
    FailureScore,
    classify_score,
...
from souplite.utils.diagnose.badge import render_badge_svg
from souplite.utils.diagnose.contamination import score_contamination
from souplite.utils.diagnose.forgetting import score_forgetting
from souplite.utils.diagnose.format import (
    is_valid_json,
    is_valid_tool_call,
    matches_regex,
    score_format,
)
from souplite.utils.diagnose.memorization import score_memorization, split_prefix
from souplite.utils.diagnose.mode_collapse import score_mode_collapse
from souplite.utils.diagnose.refusal import looks_like_refusal, score_refusal
from souplite.utils.diagnose.report import THRESHOLDS
from souplite.utils.diagnose.report import classify_score as classify_v2
from souplite.utils.diagnose.runner import (
    build_report,
    write_report,
)
from souplite.utils.diagnose.runner import (
...
class TestTrainDiagnoseGate:
    ...
    def test_run_diagnose_gate_helper_major_exits(self, tmp_path: Path) -> None:
        ...
        from souplite.commands.train import _run_diagnose_gate
    ...
    def test_diagnose_gate_skips_nonzero_local_rank(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from souplite.commands.train import _should_run_diagnose_gate_on_rank
...
class TestSourceWiring:
    def test_cli_registers_diagnose(self) -> None:
        source = (_PROJECT_ROOT / "src" / "souplite" / "cli.py").read_text(encoding="utf-8")
    ...
    def test_no_top_level_heavy_imports(self) -> None:
        ...
        probe_modules = [
            "src/souplite/utils/diagnose/__init__.py",
            "src/souplite/utils/diagnose/report.py",
            "src/souplite/utils/diagnose/_common.py",
            "src/souplite/utils/diagnose/forgetting.py",
            "src/souplite/utils/diagnose/refusal.py",
            "src/souplite/utils/diagnose/format.py",
            "src/souplite/utils/diagnose/mode_collapse.py",
            "src/souplite/utils/diagnose/memorization.py",
            "src/souplite/utils/diagnose/contamination.py",
            "src/souplite/utils/diagnose/citation.py",
            "src/souplite/utils/diagnose/badge.py",
            "src/souplite/utils/diagnose/runner.py",
        ]
...
class TestReviewFixCoverage:
    ...
    def test_write_report_uses_realpath(self) -> None:
        source = (
            _PROJECT_ROOT / "src" / "souplite" / "utils" / "diagnose" / "runner.py"
        ).read_text(
            encoding="utf-8"
        )
    ...
    def test_neutral_score_centralised(self) -> None:
        from souplite.utils.diagnose.runner import neutral_score
    ...
    def test_badge_symlink_rejected(self, tmp_path: Path) -> None:
        ...
        os.symlink(target, link)
        from souplite.commands.diagnose import _write_badge
    ...
    def test_matches_regex_invalid_pattern_rejected(self) -> None:
        ...
        source = (
            _PROJECT_ROOT / "src" / "souplite" / "utils" / "diagnose" / "format.py"
        ).read_text(
            encoding="utf-8"
        )
    ...
    def test_refusal_input_capped(self) -> None:
        from souplite.utils.diagnose.refusal import _MAX_REFUSAL_SCAN
    ...
    def test_contamination_combined_cap(self) -> None:
        ...
        source = (
            _PROJECT_ROOT / "src" / "souplite" / "utils" / "diagnose" / "contamination.py"
        ).read_text(
            encoding="utf-8"
        )
    ...
    def test_format_valid_kinds_frozenset(self) -> None:
        from souplite.utils.diagnose.format import _VALID_KINDS
    ...
    def test_tokenize_delegates(self) -> None:
        from souplite.utils._eval_text import tokenize as shared
        from souplite.utils.diagnose._common import tokenize as local
    ...
    def test_extract_row_text(self) -> None:
        from souplite.utils.diagnose._common import extract_row_text
