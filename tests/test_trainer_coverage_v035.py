...
def test_apply_v028_speed_memory_no_exception(task: str, feature: str, monkeypatch) -> None:
    ...
    from souplite.utils import v028_features as vf
...
def test_apply_v028_speed_memory_invokes_cut_ce_patcher(task: str) -> None:
    ...
    with patch("souplite.utils.cut_ce.apply_cut_ce", return_value=True):
        result = vf.apply_v028_speed_memory(
...
def test_apply_v028_speed_memory_invokes_fp8(task: str) -> None:
    ...
    tcfg = _make_tcfg("fp8")
    with patch("souplite.utils.fp8.apply_fp8_training", return_value=True):
        result = vf.apply_v028_speed_memory(
            model=MagicMock(), tcfg=tcfg,
            base_model="meta-llama/Llama-3.2-1B", console=None,
        )
...
@pytest.mark.parametrize(
    "module_path",
    (
        "souplite.trainer.sft",
        "souplite.trainer.dpo",
        "souplite.trainer.pretrain",
        "souplite.trainer.grpo",
        "souplite.trainer.kto",
        "souplite.trainer.orpo",
        "souplite.trainer.simpo",
        "souplite.trainer.ipo",
        "souplite.trainer.ppo",
...
class TestQuantNameTranslators:
    ...
    def test_quant_name_to_vllm_kwargs_known(self, name, expected) -> None:
        from souplite.utils.auto_quant import quant_name_to_vllm_kwargs
    ...
    def test_quant_name_to_bnb_kwargs(self) -> None:
        from souplite.utils.auto_quant import quant_name_to_bnb_kwargs
...
class TestFreeEngine:
    def test_free_engine_no_torch(self) -> None:
        ...
        from souplite.utils.auto_quant import free_engine
...
class TestTryReloadWithFallback:
    def _candidate(self, name: str, score: float = 0.95):
        from souplite.utils.auto_quant import Candidate

        return Candidate(name=name, score=score, latency_ms=10.0, ok=True)

    def test_picked_loads_first_try(self) -> None:
        from souplite.utils.auto_quant import try_reload_with_fallback
...
class TestVllmCreateEngineQuantizationParam:
    def test_create_vllm_engine_accepts_quantization_kwarg(self) -> None:
        ...
        from souplite.utils import vllm as vllm_mod
...
def test_benchmark_kernel_combos_cpu_returns_none_times() -> None:
    ...
    from souplite.utils.kernel_picker import benchmark_kernel_combos
