...
def clear_plugins_fixture(monkeypatch, tmp_path):
    from souplite import plugins as plugins_pkg
...
def test_build_plugin_callback_skips_disabled(clear_plugins_fixture):
    ...
    from souplite.monitoring.plugin_callback import build_plugin_callback
    from souplite.plugins import disable_plugin, register_plugin
...
def test_attach_plugin_callback_no_plugins_returns_false(
    ...
    from souplite.utils.peft_wiring import attach_plugin_callback
...
@pytest.mark.parametrize(
    "trainer_file",
    [
        "src/souplite/trainer/sft.py",
        "src/souplite/trainer/dpo.py",
        "src/souplite/trainer/grpo.py",
        "src/souplite/trainer/kto.py",
        "src/souplite/trainer/orpo.py",
        "src/souplite/trainer/simpo.py",
        "src/souplite/trainer/ipo.py",
        "src/souplite/trainer/bco.py",
        "src/souplite/trainer/ppo.py",
        "src/souplite/trainer/pretrain.py",
        "src/souplite/trainer/reward_model.py",
        "src/souplite/trainer/embedding.py",
        "src/souplite/trainer/distill.py",
    ],
)
def test_every_trainer_wires_attach_plugin_callback(trainer_file: str):
    ...
    assert "attach_plugin_callback," in src or (
        "from souplite.utils.peft_wiring import" in src
...
def test_ngram_config_threaded_into_generate_response(monkeypatch):
    ...
    from souplite.utils.ngram_spec import NgramSpecConfig
    ...
    monkeypatch.setattr("souplite.commands.serve._generate_response", _capture)
...
def test_ngram_kwarg_emits_prompt_lookup_num_tokens():
    ...
    from souplite.commands.serve import _generate_response
    from souplite.utils.ngram_spec import NgramSpecConfig
...
def test_data_recipe_execute_flag_present(tmp_path: Path):
    ...
    from souplite.cli import app as cli_app
...
def test_instantiate_trainer_plugins_in_dunder_all():
    ...
    import souplite.utils.trainer_plugins as mod
