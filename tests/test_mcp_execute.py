...
from souplite.mcp_server.execution import ExecutionError, ExecutionManager, digest_file
...
class TestSubprocessIsolationAndConcurrency:
    def test_subprocess_args_and_environment_isolation(self, tmp_path, monkeypatch):
        ...
        token = manager.issue(
            kind="train",
            argv=[sys.executable, "-m", "souplite.cli", "train"],
        ...
        with patch("subprocess.Popen") as mock_popen:
            ...
            assert call_args == [sys.executable, "-m", "souplite.cli", "train"]
    ...
    def test_capacity_released_after_tracker_launch_failure(self, tmp_path, monkeypatch):
        ...
        with patch(
            "souplite.experiment.tracker.ExperimentTracker.launch_run",
