...
from souplite.utils.layer_stream import decide_measured_fit, decide_stream_fit
...
class TestTheProbeIsActuallyWiredIntoSetup:
    ...
    def _plan(predicted, available):
        from souplite.trainer.stream_setup import _ProbePlan
    ...
    def test_an_unexpected_exception_still_releases_the_pinned_ram_store(
        ...
        wrapper = self._wrapper(monkeypatch, None)
        monkeypatch.setattr(
            "souplite.utils.layer_stream_runtime.measure_step_peak_bytes", _boom
        )
...
class TestTheDeferralHasACeiling:
    ...
    def _budget(probe, predicted_gb, free_gb, *, on_cuda=False, monkeypatch=None):
        ...
        if on_cuda:
            ...
            monkeypatch.setattr(
                "souplite.utils.layer_stream_runtime.measure_gemm_tflops",
                lambda *_a, **_k: None,
            )
