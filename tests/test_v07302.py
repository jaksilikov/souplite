...
from souplite.eval.forgetting import MINI_BENCHMARKS, extract_mcq_letter, score_answer
from souplite.eval.gate_suites import (
...
def _over_refusal():
    from souplite.eval.gate_suites import MINI_OVER_REFUSAL
...
class TestIssue346PermissiveControls:
    ...
    def test_an_echoed_menu_entry_is_not_a_tool_call(self):
        ...
        from souplite.eval.gate_suites import tool_names_in_prompt
...
class TestNoiseFloorIsPure:
    def test_identical_runs_have_a_zero_floor(self):
        from souplite.utils.ship_verdict import compute_noise_floor
...
class TestNoiseFloorChangesTheVerdict:
    def test_a_regression_under_the_floor_is_not_a_regression(self):
        from souplite.utils.ship_verdict import compute_benchmark_deltas, compute_noise_floor
    ...
    def test_a_task_win_under_the_floor_is_not_a_win(self):
        from souplite.utils.ship_verdict import TASK_AXIS, build_task_win, compute_noise_floor
    ...
    def test_without_a_floor_nothing_changes(self):
        ...
        from souplite.utils.ship_verdict import build_task_win, compute_benchmark_deltas
...
class TestNoiseFloorSurvivesTheEvidenceRoundTrip:
    ...
    def test_evidence_carries_the_floor(self):
        from souplite.utils.ship_verdict import verdict_to_evidence
    ...
    def test_replaying_the_evidence_reproduces_the_decision(self):
        from souplite.commands.ship import _verdict_from_evidence
        from souplite.utils.ship_verdict import verdict_to_evidence
    ...
    def test_a_malformed_floor_block_is_refused_not_dropped(self, block):
        ...
        from souplite.utils.ship_verdict import noise_floor_from_evidence
...
class TestUntrustedNamesCannotDriveTheTerminal:
    ...
    ``souplite.utils.terminal``, shared with every other command module)."""
...
class TestTheMcpEvidenceReaderHonoursTheFloor:
    ...
    def test_the_mcp_tool_applies_a_stored_floor(self, tmp_path, monkeypatch):
        from souplite.mcp_server.registry import tool_ship_evidence
    ...
    def test_a_malformed_floor_is_refused_by_the_mcp_reader_too(
        ...
        from souplite.mcp_server.registry import McpToolError, tool_ship_evidence
...
class TestBaselineProvenanceStamp:
    ...
    def test_matching_stamp_is_silent(self, tmp_path, monkeypatch, caplog):
        ...
        from souplite.eval.gate import resolve_baseline, write_baseline_file
    ...
    def test_unstamped_baseline_warns_unknown_provenance(self, tmp_path, monkeypatch):
        from souplite.eval.gate import resolve_baseline
    ...
    def test_mismatched_revision_warns(self, tmp_path, monkeypatch):
        ...
        from souplite.eval.gate import resolve_baseline, stamp_baseline_scores
        from souplite.eval.gate_suites import BUNDLED_SCORER_REVISION
    ...
    def test_leg2_no_longer_name_warns_on_stale_suite(self, capsys):
        ...
        from souplite.commands.ship import _leg2_scores
...
def _const_task_scorer(monkeypatch, value=0.5):
    ...
    """
    monkeypatch.setattr(
        "souplite.commands.ship._build_task_floor_scorer",
...
class TestTheFloorWideningTheThresholdIsAnnounced:
    def test_a_floor_above_the_threshold_warns(self, capsys, monkeypatch):
        from souplite.commands.ship import _measure_noise_floor
...
def _panel_text(verdict):
    ...
    from souplite.utils.ship_verdict import render_ship_panel
...
class TestThePanelPrintsItsLegOneMarker:
    ...
    def test_the_rubric_still_prints_it_too(self):
        ...
        from souplite.utils.ship_verdict import format_ship_rubric
...
class TestIssue317ThroughTheRealCli:
    ...
    def _gen(self, *, refuse_benign, wins_task):
        from souplite.eval.forgetting import build_mcq_prompt
...
class TestMeasureNoiseFloorBranches:
    ...
    def test_metric_mode_measures_the_leg_one_axis(self, tmp_path, monkeypatch):
        from souplite.commands.ship import _measure_noise_floor
        from souplite.utils.ship_verdict import TASK_AXIS
...
class TestNoiseFloorJudgeModes:
    ...
    def test_pairwise_floor_measures_the_base_against_itself(self, monkeypatch):
        from souplite.commands import ship as ship_mod
        from souplite.commands.ship import _measure_noise_floor
        from souplite.utils.ship_verdict import TASK_AXIS
...
class TestBareFunctionShapeGuard:
    ...
    def test_non_call_shapes_are_rejected(self, obj):
        from souplite.eval.gate_suites import _looks_like_a_bare_function
