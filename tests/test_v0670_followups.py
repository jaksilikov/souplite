...
class TestCmaesPlateauConvergence:
    def test_converges_on_flat_landscape(self, tmp_path, monkeypatch) -> None:
        ...
        from souplite.utils.cmaes_merge import build_cmaes_plan, run_cmaes_merge
...
class TestCmaesValidatorBoundaries:
    def test_population_min_accepted(self) -> None:
        from souplite.utils.cmaes_merge import MIN_POPULATION, validate_population_size
    ...
    def test_population_max_accepted(self) -> None:
        from souplite.utils.cmaes_merge import MAX_POPULATION, validate_population_size

        assert validate_population_size(MAX_POPULATION) == MAX_POPULATION

    def test_generations_min_accepted(self) -> None:
        from souplite.utils.cmaes_merge import MIN_GENERATIONS, validate_generations

        assert validate_generations(MIN_GENERATIONS) == MIN_GENERATIONS

    def test_generations_max_accepted(self) -> None:
        from souplite.utils.cmaes_merge import MAX_GENERATIONS, validate_generations
...
class TestVectorBankForwardCompat:
    ...
    def test_write_bank_symlink_rejected(self, tmp_path, monkeypatch) -> None:
        ...
        from souplite.utils.vector_bank import VectorBank, write_bank
...
class TestValidateUserIdBool:
    def test_bool_rejected(self) -> None:
        from souplite.utils.vector_bank import validate_user_id
...
class TestVectorBankBoundaries:
    def test_vector_dim_at_max_accepted(self) -> None:
        from souplite.utils.vector_bank import MAX_VECTOR_DIM, VectorBank
...
class TestPRRenderEmptySections:
    def test_all_empty_sections(self) -> None:
        ...
        from souplite.utils.adapter_pr import AdapterPR, render_pr_markdown
    ...
    def test_json_handles_empty_sections(self) -> None:
        from souplite.utils.adapter_pr import AdapterPR, render_pr_json
...
class TestSoupLockVersionDriftAdvisory:
    def test_version_change_not_drift(self) -> None:
        ...
        from souplite.utils.soup_lock import SoupLock, check_lock_drift
...
class TestBisectNonMonotonic:
    def test_non_monotonic_returns_valid_result(self) -> None:
        ...
        from souplite.utils.adapter_bisect import (
            BisectPlan,
            BisectResult,
            run_bisect,
        )
...
class TestSourceWiringRegressions:
    ...
    def test_all_modules_have_future_annotations(self) -> None:
        ...
        for module in (
            "src/souplite/utils/cmaes_merge.py",
            "src/souplite/utils/vector_bank.py",
            "src/souplite/utils/mole_routing.py",
            "src/souplite/utils/adapter_pr.py",
            "src/souplite/utils/soup_lock.py",
            "src/souplite/utils/adapter_bisect.py",
            "src/souplite/commands/lock.py",
        ):
    ...
    def test_supported_strategies_has_cmaes(self) -> None:
        from souplite.utils.adapter_merge import SUPPORTED_STRATEGIES
    ...
    def test_subprocess_call_uses_argv_list(self) -> None:
        ...
        root = Path(__file__).resolve().parent.parent
        src = (root / "src" / "souplite" / "commands" / "adapters.py").read_text(
            encoding="utf-8"
        )
