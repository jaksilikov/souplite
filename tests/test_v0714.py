...
class TestBranchRefKind:
    def test_branch_ref_in_valid_kinds(self):
        from souplite.registry.store import _VALID_KINDS

        assert "branch_ref" in _VALID_KINDS

    def test_attach_artifact_branch_ref(self, temp_registry, tmp_path):
        from souplite.registry.attach import attach_artifact
...
class TestBranchSchema:
    def test_registry_entry_id_field_default_none(self, temp_registry, tmp_path):
        from souplite.utils.adapter_branch import create_branch, load_branch
    ...
    def test_dataset_path_and_sha_mutually_exclusive(self, temp_registry, tmp_path):
        from souplite.utils.adapter_branch import create_branch
    ...
    def test_back_compat_v057_pointer(self, temp_registry, tmp_path):
        ...
        from souplite.utils.adapter_branch import _branches_dir, load_branch
...
class TestAttachBranchToRegistry:
    def test_happy(self, temp_registry, tmp_path):
        from souplite.registry.store import RegistryStore
        from souplite.utils.adapter_branch import (
...
class TestBranchFromRegistry:
    def test_derives_all_fields(self, temp_registry, tmp_path):
        from souplite.utils.adapter_branch import branch_from_registry
...
class TestBranchCli:
    ...
    def test_attach_to_registry_cli(self, temp_registry, tmp_path):
        ...
        from souplite.utils.adapter_branch import load_branch
...
class TestHistoryBranchEdges:
    def test_history_renders_branch_edge(self, temp_registry, tmp_path):
        ...
        from souplite.commands.history import history as history_cmd
        from souplite.utils.adapter_branch import (
            attach_branch_to_registry,
            create_branch,
        )
...
def _make_report():
    from souplite.utils.adapter_merge import MergeReport
...
class TestPredictMergedVerdict:
    def test_none_returns_unknown(self):
        from souplite.utils.adapter_merge import predict_merged_verdict
...
def _cmaes_plan(tmp_path, adapters, *, pop=4, gens=6):
    from souplite.utils.cmaes_merge import build_cmaes_plan
...
class TestBuildCmaesEvalFn:
    def test_eval_fn_merges_and_scores(self, tmp_path, monkeypatch):
        from souplite.utils.cmaes_merge import build_cmaes_eval_fn
    ...
    def test_run_cmaes_with_eval_fn_converges(self, tmp_path, monkeypatch):
        from souplite.utils.cmaes_merge import build_cmaes_eval_fn, run_cmaes_merge
    ...
    def test_eval_fn_isolates_scorer_failure(self, tmp_path, monkeypatch):
        from souplite.utils.cmaes_merge import (
...
class TestCmaesCliLive:
    ...
    def test_cmaes_runs_loop_and_writes_output(self, tmp_path, monkeypatch):
        import souplite.utils.cmaes_merge as cm
...
def loop_env(tmp_path, monkeypatch):
    ...
    from souplite.utils.loop_state import LoopState
...
class TestLoopStatePreWired:
    ...
    def test_init_state_persists(self, loop_env, tmp_path):
        from souplite.utils.loop_state import init_state, read_state
...
class TestHarvestStage:
    def test_produces_pairs(self, loop_env, tmp_path, monkeypatch):
        from souplite.utils.loop_stages import harvest_from_traces
...
class TestTrainStage:
    def test_skips_without_pairs(self, loop_env):
        from souplite.utils.loop_stages import train_dpo_from_pairs
    ...
    def test_invokes_runner(self, loop_env, tmp_path, monkeypatch):
        import souplite.utils.loop_stages as ls
...
class TestGateStage:
    def test_skipped_when_train_skipped(self, loop_env):
        from souplite.utils.loop_stages import gate_against_baseline
...
class TestDeployStage:
    def test_noop_when_not_ok(self, loop_env):
        from souplite.utils.loop_stages import deploy_to_canary
...
class TestPrewiredConfig:
    def test_build_returns_wired_config(self):
        from souplite.utils.loop_stages import (
...
class TestPrewiredE2E:
    def test_watch_harvests_and_trains(self, loop_env, tmp_path, monkeypatch):
        import souplite.utils.loop_stages as ls
        from souplite.utils.loop_iteration import list_iterations, read_iteration
        from souplite.utils.loop_state import write_state
        ...
        from souplite.utils.loop_stages import build_prewired_watch_config

        cfg = build_prewired_watch_config(
            max_iterations=1, poll_interval_sec=1.0,
        )
        from souplite.utils.loop_daemon import watch
...
class TestLoopStagesSourceWiring:
    def test_no_top_level_heavy_imports(self):
        ...
        src = (
            root / "src" / "souplite" / "utils" / "loop_stages.py"
...
class TestLoopCliPrewired:
    def _app(self):
        from souplite.commands.loop import app
    ...
    def test_init_pre_wired_flag(self, loop_env, tmp_path):
        ...
        assert result.exit_code == 0, (result.output, repr(result.exception))
        from souplite.utils.loop_state import read_state
...
def _write_iteration(iteration_id, *, served="mymodel"):
    from souplite.utils.loop_iteration import IterationRecord, write_iteration
...
class TestRegistryNameFrom:
    ...
    def test_sanitises(self, raw, expected_start):
        from souplite.utils.loop_iteration import registry_name_from
...
class TestPackIterationAsCan:
    def test_writes_can_and_entry(self, loop_env, tmp_path):
        from souplite.registry.store import RegistryStore
        from souplite.utils.loop_iteration import pack_iteration_as_can
...
class TestParsePrTarget:
    def test_happy(self):
        from souplite.utils.adapter_pr import parse_pr_target

        assert parse_pr_target("MuhtarJaksilikov/Soup#42") == (
            "MuhtarJaksilikov",
            "Soup",
            42,
        )
...
class TestResolveGithubToken:
    def test_github_token(self):
        from souplite.utils.adapter_pr import resolve_github_token
...
class TestPostPrComment:
    def test_happy_path_invokes_gh(self):
        from souplite.utils.adapter_pr import post_pr_comment
...
class TestPrCliPush:
    ...
    def test_push_happy(self, monkeypatch):
        ...
        import souplite.utils.adapter_pr as ap
...
class TestPackIterationSafely:
    def test_pack_failure_never_crashes_watch(self, loop_env, tmp_path, monkeypatch):
        ...
        import souplite.utils.loop_iteration as li
        from souplite.utils.loop_daemon import WatchConfig, watch
        from souplite.utils.loop_state import init_state
...
class TestDeploySsrfGuard:
    ...
    def test_endpoint_is_local_accepts_abbreviated_loopback(self, host):
        ...
        from souplite.utils.loop_stages import _endpoint_is_local
...
class TestNoHeavyTopLevelImports:
    ...
    @pytest.mark.parametrize("module", [
        "souplite.utils.cmaes_merge",
        "souplite.utils.loop_stages",
        "souplite.utils.loop_iteration",
        "souplite.utils.adapter_pr",
    ])
...
class TestMergeAdaptersCmaesGuard:
    def test_cmaes_strategy_rejected_with_path_hint(self, tmp_path, monkeypatch):
        from souplite.utils.adapter_merge import merge_adapters
...
class TestWriteMergedAdapterPublic:
    def test_public_symbol_and_alias(self):
        from souplite.utils import adapter_merge
...
class TestPackEntryRollback:
    def test_pack_failure_rolls_back_registry_entry(
        ...
        import souplite.cans.pack as pack_mod
        from souplite.registry.store import RegistryStore
        from souplite.utils.loop_iteration import pack_iteration_as_can
...
class TestWatchConfigValidation:
    def test_pack_iterations_must_be_bool(self):
        from souplite.utils.loop_daemon import WatchConfig
...
class TestCmaesScanGate:
    ...
    def test_cmaes_respects_backdoor_scan_gate(self, tmp_path, monkeypatch):
        ...
        import souplite.utils.adapter_scan as scan_mod
