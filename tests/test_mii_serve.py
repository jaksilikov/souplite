...
class TestServeMiiCallSite:
    ...
    * deleting `tokenizer=mii_tokenizer` from the `create_mii_pipeline(...)`
    ...
    def _invoke(self, tmp_path, *, tokenizer):
        ...
        with (
            patch("souplite.utils.mii.is_mii_available", return_value=True),
            patch(
                "souplite.utils.mii.create_mii_pipeline",
                side_effect=_fake_create_mii_pipeline,
            ),
            patch("souplite.utils.mii.build_mii_app", side_effect=_fake_build_mii_app),
            patch(
                "souplite.commands.serve._load_serve_tokenizer",
