...
class TestQwen35StreamingRuntime:
    def test_quantised_layer_suffixes_union_all_decoder_variants(self, monkeypatch):
        ...
        from souplite.utils.layer_stream_runtime import quantised_layer_suffixes
    ...
    def test_mps_gate_disables_pinning_for_direct_callers(
        ...
        import souplite.utils.layer_stream_runtime as runtime_module
        from souplite.utils.layer_shard import shard_checkpoint
    ...
    def test_build_stream_plan_accepts_actual_store_bytes_override(self):
        from souplite.utils.layer_stream import TIER_RAM, build_stream_plan
    ...
    def test_stream_layer_budget_bytes_uses_the_union_pool_spec(self, tmp_path):
        from souplite.trainer.stream_setup import StreamingSetupMixin
        from souplite.utils.layer_shard import shard_checkpoint
        from souplite.utils.layer_stream import dtype_bytes
        from souplite.utils.layer_stream_runtime import RamSource
...
class TestQwen35StreamingSetup:
    def test_stream_setup_threads_moe_targets_and_union_budget(self, monkeypatch, tmp_path):
        ...
        from souplite.utils.layer_stream import build_stream_plan as real_build_stream_plan
        ...
        monkeypatch.setattr(
            "souplite.utils.layer_shard.shard_checkpoint",
            lambda *_a, **_k: index,
        )
        monkeypatch.setattr(
            "souplite.utils.layer_shard.source_weight_bytes",
            lambda *_a, **_k: 1024,
        )
        monkeypatch.setattr(
        ...
        monkeypatch.setattr("souplite.utils.layer_stream.free_ram_bytes", lambda: 1_000_000)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            "souplite.utils.layer_stream.build_stream_plan", capture_stream_plan
        )
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            "souplite.utils.moe.detect_moe_model",
            lambda *_a, **_k: True,
        )
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            "souplite.utils.layer_stream.render_stream_panel",
            lambda *_a, **_k: "panel",
        )
        monkeypatch.setattr(
            "souplite.utils.layer_stream_runtime.expandable_segments_status",
            lambda: (True, ""),
        )
