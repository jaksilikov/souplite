...
class TestRLSignalBuffer:
    def test_record_and_snapshot(self):
        from souplite.utils.rl_signal_buffer import RLSignalBuffer
    ...
    def test_wrap_preserves_name_and_captures(self):
        from souplite.utils.rl_signal_buffer import RLSignalBuffer, wrap_reward_funcs
...
class TestRewardHackCallback:
    def test_build_returns_callback_not_notimplemented(self):
        from souplite.utils.reward_hacking import (
            RewardHackCallback,
    ...
    def test_compute_separation_from_stats(self):
        from souplite.utils.reward_hacking import compute_separation_from_stats
...
class TestEchoTrapCallback:
    def test_build_returns_callback_not_notimplemented(self):
        from souplite.utils.echo_trap import EchoTrapCallback, build_echo_trap_callback
...
class TestRLCheckpointCallback:
    ...
    def test_build_rejects_non_config(self):
        from souplite.utils.rl_checkpoint import build_rl_checkpoint_callback
...
class TestULD:
    def test_build_returns_projection(self):
        from souplite.utils.uld import ULDConfig, ULDProjection, build_uld_projection
    ...
    def test_build_rejects_non_config(self):
        from souplite.utils.uld import build_uld_projection
    ...
    def test_wasserstein_loss_different_vocab(self):
        ...
        from souplite.utils.uld import ULDConfig, uld_distill_loss
...
class TestMiniLLM:
    ...
    def test_build_rejects_non_config(self):
        from souplite.utils.minillm import build_minillm_callback
    ...
    def test_distill_term_finite_and_differentiable(self):
        ...
        from souplite.utils.minillm import MiniLLMConfig, minillm_distill_term
...
class TestIterativeDPO:
    def test_build_pairs_from_scored(self):
        from souplite.utils.iterative_dpo import build_pairs_from_scored
    ...
    def test_run_iterative_dpo_with_fakes(self, tmp_path, monkeypatch):
        from souplite.utils.iterative_dpo import (
            IterativeDPOResult,
    ...
    def test_run_rejects_non_plan(self):
        from souplite.utils.iterative_dpo import run_iterative_dpo
    ...
    def test_cli_plan_only_still_exits_zero(self, tmp_path, monkeypatch):
        ...
        from souplite.commands.iterative_dpo import app
    ...
    def test_cli_runs_with_monkeypatched_runner(self, tmp_path, monkeypatch):
        ...
        import souplite.utils.iterative_dpo as idpo
        from souplite.commands.iterative_dpo import app
        from souplite.utils.iterative_dpo import IterativeDPOResult
...
class TestGrpoVariantFallbackWarning:
    def test_fallback_warns_once(self, caplog):
        from souplite.trainer.grpo import make_grpo_trainer_variant

        cls = make_grpo_trainer_variant(_FakeGRPOBase, "gspo")
        inst = cls()
        # inputs missing per-token logps → fallback path.
        with caplog.at_level(logging.WARNING, logger="souplite.trainer.grpo"):
...
class TestGrpoEmaInPlace:
    def test_in_place_blends_toward_policy(self):
        ...
        from souplite.monitoring.grpo_stability_callback import update_ema_in_place
    ...
    def test_callback_warns_once_on_zero_overlap(self, caplog):
        ...
        from souplite.monitoring.grpo_stability_callback import (
...
class TestSourceWiring:
    def _read(self, rel: str) -> str:
        ...
        return (root / "src" / "souplite" / rel).read_text(encoding="utf-8")
