...
class TestRaftEpochCollator:
    def test_state_holds_epoch(self):
        from souplite.trainer.raft import RaftEpochState
    ...
    def test_collator_repermutes_per_epoch(self):
        ...
        from souplite.trainer.raft import RaftEpochShuffleCollator, RaftEpochState
...
class TestRaftEpochCallback:
    def test_callback_advances_epoch(self):
        from souplite.trainer.raft import RaftEpochState, make_raft_epoch_callback
...
class TestDiagnoseCitationStyle:
    def test_run_live_diagnose_threads_style(self):
        from souplite.utils.diagnose import live as live_mod
    ...
    def test_score_citation_called_with_style(self, monkeypatch):
        ...
        def _fake_score_citation(rows, generator, **kwargs):
            ...
            from souplite.utils.diagnose.report import FailureScore
        ...
        import souplite.utils.diagnose.citation as cit_mod
...
class TestBankActiveUserContextVar:
    def test_uses_contextvar(self):
        from souplite.utils import vector_bank
...
class TestMoleServeManifest:
    def _manifest(self):
        from souplite.utils.mole_routing import MoleServeManifest
    ...
    def test_write_load_roundtrip(self, tmp_path, monkeypatch):
        from souplite.utils.mole_routing import (
            load_mole_manifest,
            write_mole_manifest,
        )
    ...
    def test_load_missing_dir(self, tmp_path, monkeypatch):
        from souplite.utils.mole_routing import load_mole_manifest
...
class TestMoleTrainWritesManifest:
    def test_train_writes_manifest_source(self):
        from souplite.trainer import mole_routing as trainer_mole
...
class TestPatchInvariants:
    def test_version_bumped(self):
        ...
        assert parts >= (0, 71, 17), souplite.__version__

    @pytest.mark.parametrize(
        "module",
        ["souplite.utils.raft", "souplite.utils.mole_routing"],
    )
