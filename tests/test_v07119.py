...
_SFT_SRC = (_REPO / "src/souplite/trainer/sft.py").read_text(encoding="utf-8")
_MP_SRC = (_REPO / "src/souplite/utils/multipack_trainer.py").read_text(encoding="utf-8")
_SCHEMA_SRC = (_REPO / "src/souplite/config/schema.py").read_text(encoding="utf-8")
...
def _attach(instance):
    from souplite.utils.multipack_trainer import attach_multipack_state
...
class TestMultipackDistributedDataLoader:
    def test_routes_through_accelerator_prepare_when_distributed(self):
        ...
        from souplite.utils.multipack_sampler import MultipackBatchSampler
        from souplite.utils.multipack_trainer import make_multipack_trainer_class
...
class TestPatchInvariants:
    def test_version_bumped(self):
        ...
        assert parts >= (0, 71, 19), souplite.__version__
