...
class TestEnergyTracker:
    def test_imports(self):
        from souplite.utils.energy import EnergyMeasurement, EnergyTracker  # noqa: F401
...
class TestTopDomains:
    def test_imports(self):
        from souplite.utils.annex_xi import (  # noqa: F401
            extract_top_domains,
            load_top_domains_from_jsonl,
        )

    def test_extract_top_domains_counts_and_shares(self):
        from souplite.utils.annex_xi import extract_top_domains
    ...
    def test_load_from_jsonl(self, tmp_path, monkeypatch):
        from souplite.utils.annex_xi import load_top_domains_from_jsonl
...
class TestAnnexPdf:
    def test_atomic_write_bytes_imports(self):
        from souplite.utils.paths import atomic_write_bytes  # noqa: F401

    def test_atomic_write_bytes_roundtrip(self, tmp_path, monkeypatch):
        from souplite.utils.paths import atomic_write_bytes
    ...
    def test_render_annex_pdf_starts_with_pdf_magic(self):
        from souplite.utils.annex_xi import render_annex_pdf
    ...
    def test_write_annex_doc_markdown_default(self, tmp_path, monkeypatch):
        from souplite.utils.annex_xi import write_annex_doc
...
class TestCanManifestV3:
    def test_version_bumped_to_3(self):
        from souplite.cans.schema import (
            CAN_FORMAT_VERSION,
    ...
    def test_validate_attestation_statement_helper(self):
        from souplite.cans.schema import validate_attestation_statement
    ...
    def test_pack_entry_embeds_attestations(self, tmp_path, monkeypatch):
        ...
        from souplite.cans import pack as pack_mod
...
class TestAuditInstrumentation:
    def test_imports(self):
        from souplite.cli import _emit_audit_event, _split_command_args  # noqa: F401

    def test_split_command_args_basic(self):
        from souplite.cli import _split_command_args
    ...
    def test_emit_audit_event_writes_line(self, tmp_path, monkeypatch):
        import souplite.cli as cli_mod
    ...
    def test_emit_audit_event_never_raises(self, tmp_path, monkeypatch):
        import souplite.cli as cli_mod
        import souplite.utils.audit_log as audit_mod
    ...
    def test_subprocess_emits_audit_line(self, tmp_path):
        ...
        env["PYTHONUTF8"] = "1"
        r = subprocess.run(
            [sys.executable, "-m", "souplite", "version"],
...
class TestAirgapReproReceipt:
    def test_plan_accepts_repro_receipt_field(self, tmp_path, monkeypatch):
        from souplite.utils.airgap_bundle import AirgapBundlePlan
    ...
    def test_build_embeds_receipt_member(self, tmp_path, monkeypatch):
        ...
        from souplite.utils.airgap_bundle import (
            AirgapBundlePlan,
            build_airgap_bundle,
    ...
    def test_cli_explicit_repro_receipt(self, tmp_path, monkeypatch):
        from souplite.cli import app
        from souplite.utils.airgap_bundle import inspect_airgap_bundle
...
class TestReviewFollowups:
    ...
    def test_extract_top_domains_urls_per_row_cap(self, monkeypatch):
        import souplite.utils.annex_xi as ax
    ...
    def test_render_annex_pdf_escapes_operator_markup(self):
        from souplite.utils.annex_xi import AnnexXIData, render_annex_pdf
    ...
    def test_load_attestation_file_oversize_rejected(self, tmp_path, monkeypatch):
        from souplite.commands.can import _load_attestation_files
    ...
    def test_subprocess_no_audit_log_flag_suppresses(self, tmp_path):
        ...
        env["PYTHONUTF8"] = "1"
        r = subprocess.run(
            [sys.executable, "-m", "souplite", "--no-audit-log", "version"],
            env=env, capture_output=True, text=True, timeout=120,
        )
        assert r.returncode == 0, (r.stdout, r.stderr)
        assert not log.exists()
    ...
    def test_manifest_from_payload_coerces_bad_receipt_to_none(self):
        from souplite.utils.airgap_bundle import _manifest_from_payload
