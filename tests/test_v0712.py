...
class TestSigningPrimitives:
    def test_is_signing_available(self):
        from souplite.utils.signing import is_signing_available
    ...
    def test_private_key_from_pem_rejects_non_ed25519(self):
        ...
        from souplite.utils.signing import private_key_from_pem
    ...
    def test_load_private_key_file_missing(self, tmp_path):
        from souplite.utils.signing import load_private_key_file
    ...
    def test_resolve_signing_key_missing_raises(self):
        from souplite.utils.signing import resolve_signing_key
...
class TestAdapterSignEd25519:
    def _key(self, tmp_path):
        from souplite.utils.signing import generate_ed25519_private_pem
    ...
    def test_verify_with_matching_trusted_key(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from souplite.utils.adapter_sign import sign_adapter, verify_adapter
        from souplite.utils.signing import load_private_key_file, public_key_pem
...
class TestAttestEd25519:
    ...
    def test_sign_attestation_ed25519(self, tmp_path):
        from souplite.utils.attest import sign_attestation, verify_attestation
...
class TestNamespacePinConcurrency:
    def test_wal_mode_enabled(self, tmp_path):
        from souplite.utils.namespace_pin import NamespacePinStore
    ...
    def test_concurrent_puts_preserve_first_seen(self, tmp_path):
        ...
        from souplite.utils.namespace_pin import (
            NamespacePin,
            NamespacePinStore,
...
class TestLicenseExtraction:
    def test_from_adapter_config(self, tmp_path):
        from souplite.utils.license_matrix import extract_license_from_adapter
...
class TestLicenseOverrideAudit:
    def test_record_license_override_writes_audit(self, tmp_path, monkeypatch):
        ...
        from souplite.utils.license_matrix import record_license_override
...
class TestMergeScanGate:
    def _run(self, monkeypatch, tmp_path, args, *, scan_overall="OK", scan_raises=None):
        ...
        if scan_raises is not None:
            ...
            monkeypatch.setattr("souplite.utils.adapter_scan.scan_adapter", _scan)
        else:
            monkeypatch.setattr(
                "souplite.utils.adapter_scan.scan_adapter", _fake_scan(scan_overall)
            )
        monkeypatch.setattr(
            "souplite.utils.adapter_merge.merge_adapters",
...
class TestMergeLicenseAutoExtract:
    def _run(self, monkeypatch, tmp_path, args, audit_log=None):
        ...
        monkeypatch.setattr(
            "souplite.utils.adapter_scan.scan_adapter", _fake_scan("OK")
        )
        monkeypatch.setattr(
            "souplite.utils.adapter_merge.merge_adapters",
            lambda *a, **k: _fake_merge_report(),
...
class TestReviewFollowups:
    ...
    def test_verify_attestation_non_bytes_typeerror(self):
        from souplite.utils.attest import verify_attestation
    ...
    def test_attest_verify_non_ed25519_sidecar_exit3(self, tmp_path, monkeypatch):
        ...
        from souplite.commands.attest import app
    ...
    def test_require_cryptography_raises_when_unavailable(self, monkeypatch):
        from souplite.utils import signing

        monkeypatch.setattr(signing, "is_signing_available", lambda: False)
        with pytest.raises(ValueError, match="souplite\\[sign\\]"):
    ...
    def test_check_license_compat_empty_and_nonlist(self):
        from souplite.utils.license_matrix import check_license_compat
...
class TestSecurityFixes:
    ...
    def test_read_public_key_file_size_cap_and_symlink(self, tmp_path):
        ...
        from souplite.utils.signing import read_public_key_file
    ...
    def test_download_repo_fails_open_on_store_error(self, tmp_path, monkeypatch):
        ...
        with patch(
            "souplite.utils.namespace_pin.NamespacePinStore", boom
