...
_UTILS_DIR = Path(__file__).resolve().parent.parent / "src" / "souplite" / "utils"
...
def _imports_net_guard(path: Path) -> bool:
    """True if ``path`` has a real ``from souplite.utils.net_guard import ...``.
    ...
    """
    return any(
        isinstance(node, ast.ImportFrom) and node.module == "souplite.utils.net_guard"
