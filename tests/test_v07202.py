...
class TestNF4Sharding:
    ...
    def test_rebuilt_state_matches_the_one_bitsandbytes_built(self, tmp_path, double_quant):
        ...
        from souplite.utils.layer_stream_runtime import rebuild_quant_state
    ...
    def test_code_table_divergence_is_refused(self, tmp_path):
        ...
        from souplite.utils.layer_shard import _CodeTables
...
class TestQuantCacheInvalidation:
    ...
    def test_nf4_cache_is_not_reused_for_a_bf16_request(self, tmp_path):
        from souplite.utils.layer_shard import QUANT_NF4, QUANT_NONE, shard_checkpoint
    ...
    def test_double_quant_change_invalidates(self, tmp_path):
        from souplite.utils.layer_shard import QUANT_NF4, shard_checkpoint
...
class TestHostileIndexIsRefused:
    ...
    def _spec(self, **over):
        from souplite.utils.layer_shard import NF4WeightSpec
    ...
    def test_shape_claiming_more_than_the_packed_bytes_is_refused(self, tmp_path):
        ...
        from souplite.utils.layer_shard import QUANT_NF4, read_shard_index, shard_checkpoint
    ...
    def test_an_honest_shard_passes_the_consistency_check(self, tmp_path):
        ...
        from souplite.utils.layer_shard import ABSMAX_SUFFIX, NF4WeightSpec
        from souplite.utils.layer_stream_runtime import validate_quant_shape
    ...
    def test_quant_specs_must_agree_with_the_quant_field(self, tmp_path):
        ...
        non-4bit skeleton."""
        from souplite.utils.layer_shard import QUANT_NONE, NF4WeightSpec, ShardIndex
        from souplite.utils.layer_stream_runtime import install_streaming
...
class TestPeftDispatchesTheBnbLoraPath:
    ...
    def test_streamed_skeleton_gets_the_bnb_aware_lora_layer(self, tmp_path):
        ...
        from souplite.utils.layer_stream_runtime import build_meta_skeleton
    ...
    def test_streamed_quant_config_matches_the_resident_one(self):
        ...
        from souplite.utils.layer_stream_runtime import build_nf4_config
    ...
    def test_stream_setup_threads_double_quant_into_sharder_and_runtime(
        ...
        monkeypatch.setattr(
            "souplite.utils.layer_stream.free_ram_bytes", lambda: 10_000_000_000
        )
        monkeypatch.setattr(
            "souplite.utils.layer_stream.detect_disk_kind", lambda *_a, **_k: "nvme"
        )

        captured = {}
        import souplite.utils.layer_shard as shard_mod
        import souplite.utils.layer_stream_runtime as rt_mod
...
class TestNF4SpecFromShard:
    ...
    def test_zero_dim_offset_keeps_its_shape(self, tmp_path):
        from souplite.utils.layer_shard import NESTED_OFFSET_SUFFIX
        from souplite.utils.layer_stream_runtime import RamSource
...
class TestNF4StreamedModel:
    ...
    def test_pool_streams_the_quant_sidecars(self, tmp_path):
        from souplite.utils.layer_shard import ABSMAX_SUFFIX
    ...
    def test_rebuilt_view_shares_storage_with_the_pooled_buffer(self, tmp_path):
        ...
        from souplite.utils.layer_stream_runtime import decoder_owner, rebuild_params4bit
...
class TestNF4StoreEstimate:
    def test_plain_dtype_is_unscaled(self):
        from souplite.utils.layer_stream import estimate_stream_store_bytes
...
class TestPreflightUsesTheStreamedSize:
    ...
    def _run(
        ...
        monkeypatch.setattr(
            "souplite.utils.layer_shard.source_weight_bytes", lambda *_a, **_k: on_disk
        )
        ...
        monkeypatch.setattr(
            "souplite.utils.layer_stream.detect_disk_kind", lambda *_a, **_k: disk_kind
        )
        cfg = load_config_from_string(
            f"""
base: {weights}
...
class TestDeviceMapValue:
    ...
    def test_indexed_devices_become_ints(self, device, expected):
        from souplite.utils.layer_stream_runtime import _device_map_value
