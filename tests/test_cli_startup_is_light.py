"""The light-CLI invariant: ``import souplite.cli`` must not load PyTorch.
...
def test_cli_import_does_not_load_torch():
    ...
    assert not leaked, (
        f"`import souplite.cli` pulled in {leaked}. The light core must stay "
...
def test_probe_would_catch_a_regression():
    ...
    proc = subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys, torch; import souplite.cli; "
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "souplite.utils.reward_stress",
        "souplite.utils.reward_synth",
        "souplite.utils.ship_verdict",
        "souplite.utils.layer_stream",
        ...
        "souplite.mcp_server.registry",
        "souplite.utils.advise",
        "souplite.eval.gate_suites",
        "souplite.recipes.catalog",
    ],
)
...
def test_every_command_module_is_imported_at_startup():
    """Pins WHY the single ``import souplite.cli`` assertion is sufficient.
    ...
    probe = (
        "import pathlib, sys\n"
        "import souplite.cli\n"
        "loaded = {m for m in sys.modules if m.startswith('souplite.commands.')}\n"
        "root = pathlib.Path(souplite.cli.__file__).parent / 'commands'\n"
        "disk = {'souplite.commands.' + p.stem for p in root.glob('*.py')"
        " if p.stem != '__init__'}\n"
...
@pytest.mark.parametrize(
    "module",
    [
        "souplite.utils.reward_hack_control",
        "souplite.utils.reward_hacking",
        "souplite.utils.echo_trap",
        "souplite.utils.minillm",
        "souplite.utils.rl_checkpoint",
        "souplite.utils.relora",
        "souplite.utils.lisa",
        "souplite.monitoring.hf_push",
        "souplite.monitoring.curriculum_callback",
        "souplite.monitoring.grpo_stability_callback",
        "souplite.monitoring.callback",
    ],
)
...
def test_stress_sentinel_matches_the_controller():
    ...
    """
    from souplite.utils import reward_hack_control, reward_stress
