...
from souplite.utils import expectations
...
class TestSourceWiring:
    def test_cli_registers_expect(self) -> None:
        ...
        cli = (root / "src" / "souplite" / "cli.py").read_text(encoding="utf-8")
        assert 'name="expect"' in cli or "from souplite.commands import expect" in cli

    def test_no_heavy_top_level_imports(self) -> None:
        root = Path(__file__).resolve().parent.parent
        src = (root / "src" / "souplite" / "utils" / "expectations.py").read_text(encoding="utf-8")
        for forbidden in (
            "\nimport torch",
            "\nimport transformers",
            "\nimport peft",
