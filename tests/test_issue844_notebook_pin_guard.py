...
- the install cell pins an exact `souplite[train]==X.Y.Z`, never a `git+` URL;
- that `X.Y.Z` has a `## [X.Y.Z]` heading in CHANGELOG.md, i.e. it is a
  released version and not whatever `main` happened to be that day;
- every `from souplite.<module> import <name>` in a code cell still resolves
...
PIN_RE = re.compile(r'souplite\[train\]\s*==\s*(?P<version>\d+\.\d+\.\d+)')
GIT_INSTALL_RE = re.compile(r'souplite\[train\]\s*@\s*git\+')
...
def _install_cell_source():
    for src in _code_cell_sources():
        if "%pip install" in src and "souplite" in src:
            return src
    pytest.fail("no code cell installs souplite")
...
class TestTheInstallCellIsPinnedToARelease:
    def test_it_does_not_install_from_git(self):
        ...
        assert not GIT_INSTALL_RE.search(src), (
            "the install cell pulls souplite from a git ref, which is not "
    ...
    def test_it_pins_an_exact_version(self):
        ...
        assert match, (
            'expected `souplite[train]==X.Y.Z` in the install cell, found '
            f'none in:\n{src}'
        )
    ...
    def test_the_pinned_version_is_actually_released(self):
        ...
        assert heading in changelog, (
            f"notebook pins souplite=={version}, but {CHANGELOG_PATH.name} has "
...
class TestEveryImportInTheNotebookStillResolves:
    ...
    def _souplite_imports(self):
        ...
        for src in _code_cell_sources():
            try:
                ...
                # `from souplite...` imports anyway.
                continue
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom) and node.module:
                    if node.module == "souplite" or node.module.startswith("souplite."):
                        for alias in node.names:
                            imports.append((node.module, alias.name))
        return imports

    def test_notebook_actually_imports_something_from_souplite(self):
        assert self._souplite_imports(), (
            "expected at least one `from souplite...` import in the "
    ...
    def test_each_imported_name_resolves_against_the_current_tree(self):
        failures = []
        for module_name, attr_name in self._souplite_imports():
            try:
