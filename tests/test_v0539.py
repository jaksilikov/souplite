...
def test_version_bump_to_0_53_9():
    ...
    parts = tuple(int(p) for p in souplite.__version__.split(".") if p.isdigit())
    assert parts >= (0, 53, 9)
...
def test_train_event_buffer_push_and_drain():
    ...
    from souplite.utils.train_event_buffer import TrainEventBuffer
...
def test_push_train_event_silent_on_bad_input():
    from souplite.utils.train_event_buffer import (
        get_global_buffer,
        push_train_event,
        reset_global_buffer,
    )
...
def _auth_headers():
    from souplite.ui.app import get_auth_token
...
def test_api_train_stream_emits_pending_events():
    ...
    from souplite.ui.app import create_app
    from souplite.utils.sse_train_stream import TrainEvent
    from souplite.utils.train_event_buffer import (
        push_train_event,
        reset_global_buffer,
...
def test_detect_backend_fallback_transformers():
    from souplite.utils.backend_detect import detect_backend
...
def test_set_auth_token_concurrent_rotation_safe():
    ...
    from souplite.ui.app import get_auth_token, set_auth_token
