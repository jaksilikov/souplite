...
class TestMixExtra:
    ...
    def test_describe_default_optimizer_returns_label(self):
        from souplite.utils.data_mix import describe_default_optimizer
...
class TestPostHogEnvOverride:
    def test_resolve_uses_default_key_when_env_unset(self):
        from souplite.utils.trackers import _POSTHOG_DEFAULT_KEY, _resolve_posthog_target
    ...
    def test_resolve_env_key_overrides_default(self):
        from souplite.utils.trackers import _resolve_posthog_target
    ...
    def test_resolve_explicit_endpoint_locks_against_env(self):
        ...
        from souplite.utils.trackers import _POSTHOG_ENDPOINT, _resolve_posthog_target
...
class TestHubPrefetchHelper:
    ...
    def test_prefetch_outside_cwd_cache_root_raises(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        from souplite.utils.hubs import prefetch_model_from_hub
...
class TestCliHubFlags:
    @pytest.mark.parametrize(
        "cmd_module,cmd_name",
        [
            ("souplite.commands.chat", "chat"),
            ("souplite.commands.serve", "serve"),
            ("souplite.commands.infer", "infer"),
            ("souplite.commands.merge", "merge"),
            ("souplite.commands.export", "export"),
            ("souplite.commands.push", "push"),
        ],
    )
    ...
    def test_apply_hub_helper_imported_in_non_push_commands(self):
        ...
        for mod in ("chat", "serve", "infer", "merge", "export"):
            src = (REPO_ROOT / f"src/souplite/commands/{mod}.py").read_text(
                encoding="utf-8"
            )
    ...
    def test_push_uses_upload_repo_path(self):
        ...
        src = (REPO_ROOT / "src/souplite/commands/push.py").read_text(
            encoding="utf-8"
        )
...
class TestDataDownloadNonHfLive:
    ...
    def test_data_download_no_longer_advises_v0_53_9(self):
        ...
        src = (REPO_ROOT / "src/souplite/commands/data.py").read_text(
            encoding="utf-8"
        )
...
class TestWebUiToolOutputsPanel:
    def test_index_html_has_tools_nav_entry(self):
        html = (REPO_ROOT / "src/souplite/ui/static/index.html").read_text(
            encoding="utf-8"
        )
    ...
    def test_app_js_has_load_tool_outputs(self):
        js = (REPO_ROOT / "src/souplite/ui/static/app.js").read_text(encoding="utf-8")
...
class TestCallbackToolBuffer:
    ...
    def test_on_step_end_no_op_when_inputs_absent(self):
        from souplite.monitoring.callback import SoupTrainerCallback
        from souplite.utils.tool_outputs import (
