...
That is #752, and it surfaced after `soup eval auto` had already run the eval
...
def test_the_package_declares_typer_commands_at_all():
    ...
    key = ("souplite.commands.eval", "custom")
...
def test_the_walk_reaches_the_whole_package_not_just_commands():
    ...
    assert "souplite.monitoring.callback" in modules
