...
from souplite.recipes.catalog import RECIPES, get_recipe
from souplite.utils.curriculum_dynamic import DynamicCurriculumPolicy
from souplite.utils.data_mix import (
    _build_skopt_optimizer,
    build_optimization_plan,
    run_mix_optimizer,
)
from souplite.utils.mix_proxy import proxy_run_for_weights
...
def test_trainer_source_grep_curriculum_wired_everywhere():
    ...
    root = Path(__file__).resolve().parent.parent / "src" / "souplite" / "trainer"
    trainers = [
...
def test_overlay_yaml_loads_through_config_schema(tmp_path):
    ...
    from souplite.utils.mix_proxy import _render_overlay_yaml
...
def test_proxy_happy_path_reads_tracker(tmp_path, monkeypatch):
    ...
    with patch.object(subprocess, "run", return_value=fake_result), \
            patch(
                "souplite.utils.mix_proxy.ExperimentTracker",
                return_value=fake_tracker,
                create=True,
            ):
        # We need to patch the lazy import inside `_read_final_eval_loss`:
        import souplite.experiment.tracker as tracker_mod
