...
class TestPromptStrategyRuntime:
    def test_resolve_finds_callable(self):
        from souplite.utils.data_pipeline import resolve_prompt_strategy
    ...
    def test_apply_with_none_returns_row(self):
        from souplite.utils.data_pipeline import apply_prompt_strategy
        row = {"a": 1}
...
class TestForgeJudgeProvider:
    def test_make_judge_provider_fn_unknown_rejected(self):
        from souplite.utils.data_forge import make_judge_provider_fn
...
class TestRunRecipeLive:
    def test_stub_gone(self, tmp_path, monkeypatch):
        from souplite.utils.recipe_dag import parse_recipe
...
class TestInstantiateTrainerPlugins:
    def test_cce_plugin_returns_advisory_callback(self):
        from souplite.utils.trainer_plugins import instantiate_trainer_plugins
...
class TestToolEndpointsLive:
    ...
    def test_bash_tool_executes_simple_command(self, monkeypatch):
        ...
        from souplite.trainer.rewards import SandboxProcessResult

        app = _create_test_app()
        client = TestClient(app, base_url="http://127.0.0.1")
        monkeypatch.setattr(
            "souplite.trainer.rewards._get_isolation_strategy", lambda: "namespaces"
        )
        monkeypatch.setattr(
            "souplite.trainer.rewards._run_bash_sandbox",
    ...
    def test_bash_tool_blocks_network_namespace(self):
        ...
        from souplite.trainer.rewards import _get_isolation_strategy, _run_bash_sandbox
    ...
    def test_bash_tool_windows_returns_501(self, monkeypatch):
        ...
        client = TestClient(app, base_url="http://127.0.0.1")
        monkeypatch.setattr(
            "souplite.trainer.rewards._get_isolation_strategy", lambda: "best-effort"
        )
        resp = client.post(
            "/v1/tools/bash",
            json={"command": "echo hello"},
    ...
    def test_bash_tool_bounded_streaming_kills_massive_output(self):
        if sys.platform == "win32":
            pytest.skip("bash sandbox not supported on Windows")
        from souplite.trainer.rewards import _run_sandboxed_subprocess
    ...
    def test_bash_sandbox_raises_on_strict_namespace_failure(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "souplite.trainer.rewards._get_isolation_strategy",
            lambda: "namespaces",
        )
        ...
        from souplite.trainer.rewards import _run_bash_sandbox
    ...
    def test_bash_sandbox_requests_strict_namespaces_by_name(self, monkeypatch):
        ...
        from souplite.trainer import rewards
    ...
    def test_web_search_with_allowlist_calls_backend(self):
        ...
        from souplite.commands.serve import _create_app
        from souplite.utils.server_tools import WebSearchConfig
...
class TestAnthropicMessagesStreaming:
    ...
    def test_stream_true_returns_sse(self):
        ...
        with patch(
            "souplite.commands.serve._generate_response",
...
class TestVllmAnthropicMessages:
    ...
    def test_vllm_stream_helper_emits_anthropic_events(self):
        from souplite.utils.vllm import _stream_anthropic_messages_vllm
...
class TestServeStreamHelper:
    def test_transformers_stream_helper_emits_anthropic_events(self):
        from souplite.commands.serve import _stream_anthropic_messages
...
class TestPublicSurface:
    def test_data_pipeline_exports(self):
        from souplite.utils import data_pipeline
        for name in (
    ...
    def test_data_forge_exports(self):
        from souplite.utils import data_forge
    ...
    def test_trainer_plugins_exports(self):
        from souplite.utils import trainer_plugins
...
class TestReviewFixesRecipeRun:
    ...
    def test_node_code_row_injection_blocked_by_double_encode(self, tmp_path, monkeypatch):
        ...
        from souplite.utils.recipe_dag import RecipeNode
        from souplite.utils.recipe_run import _node_code
        monkeypatch.chdir(tmp_path)
    ...
    def test_run_recipe_resume_rehydrates_predecessor_outputs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        from souplite.utils.recipe_dag import RecipeDAG, RecipeNode
        from souplite.utils.recipe_run import run_recipe
    ...
    def test_save_checkpoint_atomic_via_mkstemp(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        from souplite.utils.recipe_run import _save_checkpoint
        outdir = tmp_path / "out"
        outdir.mkdir()
...
class TestReviewFixesVllmAnthropicLive:
    ...
    def _build_vllm_app(self):
        ...
        from souplite.utils.vllm import create_vllm_app
...
class TestReviewFixesDataScore:
    ...
    def test_load_jsonl_rows_rejects_symlink(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            from souplite.utils.data_score import load_jsonl_rows
    ...
    def test_extract_row_text_publicly_exposed(self):
        ...
        from souplite.utils import data_score
        from souplite.utils.data_score import extract_row_text
...
class TestReviewFixesTrainerPlugins:
    ...
    def test_validate_trainer_plugin_list_rejects_duplicates(self):
        ...
        from souplite.utils.trainer_plugins import validate_trainer_plugin_list
...
class TestReviewFixesSSEHeaders:
    ...
    def test_sanitise_sse_field_strips_crlf_nul(self):
        from souplite.commands.serve import _sanitise_sse_field
...
class TestReviewFixesPreprocessAtomic:
    ...
    def test_preprocess_save_uses_atomic_pattern(self):
        src = Path(__file__).parent.parent / "src" / "souplite" / "commands" / "data.py"
...
class TestSftPretrainPreTokenizedShortCircuit:
    ...
    def test_pretrain_short_circuits_to_arrow_dataset(self, tmp_path, monkeypatch):
        ...
        from souplite.trainer.sft import _maybe_load_pretokenized as helper
