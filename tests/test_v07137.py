"""v0.71.37 — every printed `pip install souplite[extra]` hint must be cmd.exe-safe.
...
pip rejects the requirement outright:
    ...
    ERROR: Invalid requirement: "'souplite[train]'": Expected package name at
    the start of dependency specifier

Measured on Windows before this fix:

    cmd.exe      'souplite[train]' -> ERROR   "souplite[train]" -> ok   bare -> ok
    PowerShell   'souplite[train]' -> ok      "souplite[train]" -> ok

Double quotes are the only spelling that works in *every* shell, which is why
the repo already says `pip install -e ".[dev]"`. Bare `souplite[train]` is not
...
# `pip install "souplite[x]"` and the Rich-escaped `pip install "souplite\[x]"`.
...
SINGLE_QUOTED = re.compile(r"pip install 'souplite\\*\[[a-z][a-z0-9-]*\]'")


def _package_root() -> pathlib.Path:
    import souplite

    return pathlib.Path(souplite.__file__).parent


def _repo_root() -> pathlib.Path | None:
    """src-layout: <repo>/src/souplite. None when installed as a plain wheel."""
...
class TestHintsAreCmdSafe:
    def test_no_single_quoted_hint_in_package(self):
        """Nothing under src/souplite may print the cmd-hostile spelling."""
        root = _package_root()
        offenders = []
        ...
        assert not offenders, (
            "single-quoted souplite[extra] hint — cmd.exe passes the quotes to "
            'pip and it errors out. Use \\"souplite[extra]\\" (works in every '
            "shell):\n" + "\n".join(offenders)
        )
        ...
        for path in targets:
            ...
            for lineno, line in enumerate(
                ...
                if in_fence and SINGLE_QUOTED.search(line):
                    offenders.append(f"{rel}:{lineno}: {line.strip()}")
        assert not offenders, (
            "single-quoted souplite[extra] in a docs code block — a Windows "
    ...
    def test_regex_actually_matches_the_broken_spelling(self):
        ...
        the plain `'souplite[fast]'` sites but not a single Rich-escaped one --
        ...
        assert SINGLE_QUOTED.search("pip install 'souplite[train]'")
        # Rich-escaped, exactly as it sits in ui.py: TWO backslash chars.
        assert SINGLE_QUOTED.search(
            '"Install with: [bold]pip install \'souplite\\\\[ui]\'[/]"'
        )
        # The spellings we are migrating *to* must not be flagged.
        assert not SINGLE_QUOTED.search('pip install "souplite[train]"')
        assert not SINGLE_QUOTED.search('pip install "souplite\\\\[ui]"')
        assert not SINGLE_QUOTED.search("pip install souplite")
...
class TestDoubleQuotesSurviveRich:
    ...
    def test_rich_keeps_escaped_bracket_inside_double_quotes(self):
        ...
        assert render('[bold]pip install "souplite\\[ui]"[/]') == (
