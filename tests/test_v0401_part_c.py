...
def test_guess_params_unknown_model_falls_back_to_1b():
    from souplite.autopilot.analyzer import _guess_params_from_name
...
def test_probe_cache_returns_none_for_missing_repo():
    from souplite.autopilot.analyzer import _probe_cache_param_count
...
def test_version_ge_handles_dev_suffix():
    from souplite.commands.doctor import _version_ge
...
def test_max_exclusive_table_allows_transformers5_and_caps_transformers6():
    from souplite.commands.doctor import _MAX_EXCLUSIVE
...
def test_live_lr_sweep_uses_load_raw_data_not_load_local():
    ...
    from souplite.commands.train import _live_lr_sweep_from_config

    src = inspect.getsource(_live_lr_sweep_from_config)
    assert "load_raw_data" in src
    # The broken import was `from souplite.data.loader import load_local`;
    # match only that import line to avoid false positives on the comment
    # describing the fix.
    assert not re.search(r"from\s+souplite\.data\.loader\s+import\s+load_local", src), (
...
def test_pick_quickstart_model_no_cuda_uses_tinyllama():
    from souplite.commands import quickstart as qs
...
def test_detect_gpu_hw_without_torch_cuda_no_nvidia_smi():
    ...
    from souplite.commands.doctor import _detect_gpu_hw_without_torch_cuda
...
def test_nvidia_smi_invoked_by_absolute_path():
    ...
    from souplite.commands.doctor import (
...
def test_parse_cuda_version_from_nvidia_smi_header():
    from souplite.commands.doctor import _parse_cuda_version
...
def test_detect_gpu_hw_distinguishes_cuda_build_from_cpu_build(monkeypatch):
    ...
    monkeypatch.setattr(
        "souplite.commands.doctor._nvidia_smi_executable",
...
def test_torch_cuda_wheel_tag_picks_supported_index():
    from souplite.commands.doctor import _TORCH_CUDA_WHEELS, _torch_cuda_wheel_tag
...
def test_detect_dual_python_no_path_python():
    from souplite.commands.doctor import _detect_dual_python_interpreters
