...
_LIVE = {
    "sft": ("souplite.trainer.sft", "SFTTrainerWrapper", _sft_rows, {}),
    "bco": ("souplite.trainer.bco", "BCOTrainerWrapper", _pref_rows, {}),
    "dpo": ("souplite.trainer.dpo", "DPOTrainerWrapper", _pref_rows, {}),
    "ipo": ("souplite.trainer.ipo", "IPOTrainerWrapper", _pref_rows, {}),
    "kto": ("souplite.trainer.kto", "KTOTrainerWrapper", _kto_rows, {}),
    "orpo": ("souplite.trainer.orpo", "ORPOTrainerWrapper", _pref_rows, {}),
    "simpo": ("souplite.trainer.simpo", "SimPOTrainerWrapper", _pref_rows, {}),
    "reward_model": (
    ...
    "classifier": (
        "souplite.trainer.classifier",
        "ClassifierTrainerWrapper",
        _label_rows,
        {"num_labels": 2},
    ),
    "pretrain": (
        "souplite.trainer.pretrain",
...
class TestEveryConfigClassAcceptsTheSeed:
    ...
    def _classes(self):
        ...
        from souplite.trainer._trl_compat import resolve_trl_symbol
    ...
    def test_the_installed_configs_accept_the_field(self, field):
        _requires_train_extra()
        from souplite.trainer._trl_compat import config_accepts
...
def _modules_defining_setup():
    """`souplite.trainer` module name -> its `setup()` AST node.
    ...
    spec = importlib.util.find_spec("souplite.trainer")
    found = {}
...
class TestEveryWrapperAppliesTheSeed:
    ...
    def _setup_body(name):
        ...
        assert name in setups, f"souplite.trainer.{name} defines no setup()"
        return setups[name]
    ...
    def test_the_derivation_found_the_wrappers(self):
        ...
        assert {"sft", "grpo", "orpo", "unlearn"} <= set(_MODULES), (
            f"derived only {sorted(_MODULES)} from souplite.trainer, which "
    ...
    def test_the_wrapper_seeds_inside_setup(self, name):
        ...
        assert seeded, (
            f"souplite.trainer.{name}.setup() never calls apply_training_seed(), "
    ...
    def test_the_seed_is_applied_before_the_model_is_built(self, name):
        ...
        assert built, (
            f"souplite.trainer.{name}.setup() has no recognisable model build; "
        ...
        assert min(seeded) < min(built), (
            f"souplite.trainer.{name}: apply_training_seed() is called at line "
    ...
    def test_the_wrapper_threads_the_seed_into_its_config(self, name):
        ...
        spec = importlib.util.find_spec(f"souplite.trainer.{name}")
        source = open(spec.origin, encoding="utf-8").read()
        assert "training_seed_kwargs(" in source, (
            f"souplite.trainer.{name} builds a TrainingArguments subclass "
...
class TestSeedingHelper:
    ...
    def test_kwargs_carry_both_fields(self):
        from souplite.utils.seeding import training_seed_kwargs
    ...
    def test_applying_the_seed_makes_torch_reproducible(self):
        ...
        from souplite.utils.seeding import apply_training_seed
    ...
    def test_the_module_imports_without_torch(self):
        ...
        code = (
            ...
            "import souplite.utils.seeding as s;"
