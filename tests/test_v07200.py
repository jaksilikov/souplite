...
class TestStreamArchAllowlist:
    def test_llama_family_supported(self):
        from souplite.utils.layer_stream import stream_arch_of
...
class TestChooseTier:
    def test_fits_in_ram_returns_ram(self):
        from souplite.utils.layer_stream import TIER_RAM, choose_tier
    ...
    def test_headroom_is_strict_at_the_boundary(self):
        ...
        from souplite.utils.layer_stream import TIER_DISK, choose_tier
    ...
    def test_hdd_is_refused_loudly(self):
        ...
        from souplite.utils.layer_stream import choose_tier
...
class TestDecidePinning:
    ...
    def test_store_within_limit_is_pinned(self):
        from souplite.utils.layer_stream import decide_pinning
...
class TestBufferValidation:
    def test_default_is_two(self):
        from souplite.utils.layer_stream import DEFAULT_STREAM_BUFFERS

        assert DEFAULT_STREAM_BUFFERS == 2

    def test_valid_range_accepted(self):
        from souplite.utils.layer_stream import validate_stream_buffers
...
class TestThroughputEstimate:
    def test_uses_c_equals_six_for_gradient_checkpointing(self):
        ...
        from souplite.utils.layer_stream import FLOPS_PER_PARAM_PER_TOKEN

        assert FLOPS_PER_PARAM_PER_TOKEN == 6

    def test_hand_computed_rate(self):
        from souplite.utils.layer_stream import estimate_stream_tokens_per_sec
    ...
    def test_epoch_seconds(self):
        from souplite.utils.layer_stream import estimate_epoch_seconds
...
class TestVramEstimate:
    def test_sums_the_plan_41_terms(self):
        from souplite.utils.layer_stream import estimate_stream_vram
    ...
    def test_logits_budget_includes_the_whole_loss_path(self):
        ...
        from souplite.utils.layer_stream import estimate_logits_bytes
...
class TestLayerSpec:
    def test_nbytes_from_shape_and_dtype(self):
        from souplite.utils.layer_stream import LayerSpec
...
class TestStreamPlan:
    def test_build_plan_reports_tier_and_pinning(self):
        from souplite.utils.layer_stream import build_stream_plan
    ...
    def test_render_panel_names_the_beta_status_and_tier(self):
        ...
        from souplite.utils.layer_stream import build_stream_plan, render_stream_panel
...
class TestNoTopLevelTorch:
    ...
    def test_layer_stream_has_no_top_level_torch(self):
        import souplite.utils.layer_stream as mod
    ...
    def test_importable_without_torch_installed(self):
        ...
        try:
            importlib.reload(importlib.import_module("souplite.utils.layer_stream"))
        finally:
...
class TestShardRoundTrip:
    ...
    def test_tied_embedding_stays_resident_and_unaffected(self, tmp_path):
        ...
        from souplite.utils.layer_shard import extras_shard_path, shard_checkpoint
    ...
    def test_untied_vocabulary_pair_is_split_from_resident_extras(self, tmp_path):
        ...
        from souplite.utils.layer_shard import (
            extras_shard_path,
            large_shard_path,
            shard_checkpoint,
        )
    ...
    def test_index_round_trips(self, tmp_path):
        from souplite.utils.layer_shard import read_shard_index, shard_checkpoint
...
class TestShardGuards:
    ...
    def test_too_many_layers_refused(self, tmp_path, monkeypatch):
        import souplite.utils.layer_shard as mod
...
class TestShardCacheDir:
    def test_slug_is_traversal_safe(self):
        from souplite.utils.layer_shard import model_slug
    ...
    def test_env_override_inside_tmp_is_honoured(self, tmp_path, monkeypatch):
        from souplite.utils.layer_shard import resolve_shard_dir
...
class TestRegressionAttributeTransparency:
    ...
    def test_attention_type_is_visible_when_the_inner_layer_has_it(self, tmp_path):
        ...
        from souplite.utils.layer_stream_runtime import StreamedDecoderLayer
...
class TestBufferPool:
    def test_slot_round_robin(self):
        from souplite.utils.layer_stream_runtime import LayerBufferPool
    ...
    def test_wait_after_load_returns_the_buffer(self, tmp_path):
        from souplite.utils.layer_shard import shard_checkpoint
        from souplite.utils.layer_stream_runtime import LayerBufferPool, RamSource
...
class TestPrefetcherDirection:
    ...
    def test_forward_prefetches_ascending(self):
        from souplite.utils.layer_stream_runtime import StreamPrefetcher
...
class TestExpandableSegments:
    ...
    def test_returns_a_bool(self):
        from souplite.utils.layer_stream_runtime import probe_expandable_segments
...
class TestStreamedForwardParityCpu:
    ...
    def test_all_aliases_of_a_tied_boundary_module_are_replaced(self):
        ...
        from souplite.utils.layer_stream_runtime import _replace_module_references
...
class TestRuntimeNoTopLevelTorch:
    def test_runtime_is_torch_lazy(self):
        import souplite.utils.layer_stream_runtime as mod
...
class TestHfGradientCheckpointingInteraction:
    ...
    def test_sft_guards_the_hf_flag_with_the_helper(self):
        ...
        from souplite.trainer import sft
...
class TestSourceSizeProbe:
    ...
    def test_reports_the_source_byte_size(self, tmp_path):
        from souplite.utils.layer_shard import source_weight_bytes
...
class TestDiskTierPinsItsStagingOrRefuses:
    ...
    def _patch_async_source(self, monkeypatch, *, fails_when_pinned: bool):
        ...
        Patched on `souplite.utils.async_disk_source` because `_build_source`
...
class TestCachedIndexInvalidation:
    ...
    def test_missing_large_layer_shard_reshards(self, tmp_path):
        from souplite.utils.layer_shard import large_shard_path, shard_checkpoint
...
class TestMaterializeExtrasGuard:
    def test_incomplete_extras_shard_is_refused(self, tmp_path):
        ...
        from souplite.utils.layer_shard import extras_shard_path, shard_checkpoint
        from souplite.utils.layer_stream_runtime import build_meta_skeleton, materialize_extras
...
class TestHardwareFitGateIsStreamingAware:
    ...
    def test_resident_run_is_still_gated(self):
        ...
        from souplite.commands.train import _hardware_fit_preflight
