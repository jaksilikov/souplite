...
def test_late_failure_resumes_without_replaying_completed_prefix(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("souplite.utils.magpie.make_magpie_generate_fn", make_generate)
    monkeypatch.setattr("souplite.eval.judge.JudgeEvaluator", lambda **_kwargs: Judge())
...
def test_resume_rejects_changed_run_before_sampling(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("souplite.utils.magpie.make_magpie_generate_fn", make_generate)
    monkeypatch.setattr(
        "souplite.eval.judge.JudgeEvaluator",
...
def test_final_datasets_use_exact_utf8_bytes_for_manifest_hashes(tmp_path, monkeypatch):
    from souplite.commands.data import app
    from souplite.utils.paths import atomic_write_text as real_text_write
    ...
    monkeypatch.setattr(
        "souplite.utils.paths.atomic_write_text", reject_dataset_text_writes
    )
    result = CliRunner().invoke(app, _args(tmp_path))
...
def test_final_publication_rolls_back_as_a_group_on_member_failure(
    ...
    from souplite.commands.data import app
    from souplite.utils.paths import atomic_write_bytes as real_bytes_write
    ...
    monkeypatch.setattr(
        "souplite.utils.paths.atomic_write_bytes", fail_group_write
    )
    result = CliRunner().invoke(app, _args(tmp_path))
...
def test_checkpoint_rejects_duplicate_or_reordered_indexes(tmp_path, monkeypatch):
    from souplite.utils.best_of_n_checkpoint import load_checkpoint
...
def test_checkpoint_entry_digest_detects_parseable_corruption(tmp_path, monkeypatch):
    from souplite.utils.best_of_n_checkpoint import (
        append_checkpoint,
...
def test_local_sampling_matches_uninterrupted_run_across_prompt_indices(
    ...
    monkeypatch.setattr(
        "souplite.commands.data._load_bon_model", lambda *_args: (object(), object())
    )
    monkeypatch.setattr(
        "souplite.utils.trust_remote.model_requires_trust_remote_code",
        lambda _model: False,
    ...
    monkeypatch.setattr(
        "souplite.eval.judge.JudgeEvaluator", lambda **_kwargs: Judge()
    )
