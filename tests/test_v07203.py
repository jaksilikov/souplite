...
from souplite.utils import layer_stream, layer_stream_runtime
...
def _predict(row):
    from souplite.utils.layer_stream import estimate_stream_peak_vram
...
class TestActivationBytes:
    def test_boundary_term_scales_with_layers_but_transient_does_not(self):
        ...
        from souplite.utils.layer_stream import estimate_activation_bytes
...
class TestStreamFitDecision:
    def test_refuses_when_demand_exceeds_available(self):
        from souplite.utils.layer_stream import decide_stream_fit
...
class TestResolveAvailableVramBytes:
    ...
    def test_no_override_uses_the_measured_figure(self):
        from souplite.utils.layer_stream import resolve_available_vram_bytes
    ...
    def test_override_below_real_free_vram_makes_a_fitting_config_refused(self):
        ...
        from souplite.utils.layer_stream import decide_stream_fit, resolve_available_vram_bytes
...
class TestThroughputForecast:
    def test_ceiling_uses_c_equals_six(self):
        from souplite.utils.layer_stream import forecast_stream_throughput
...
class TestMeasuredGemmCeiling:
    def test_returns_a_plausible_tflops_and_the_clock_it_was_taken_at(self):
        ...
        from souplite.utils.layer_stream_runtime import measure_gemm_tflops
    ...
    def test_the_reported_rate_matches_an_independently_timed_matmul(self):
        ...
        from souplite.utils.layer_stream_runtime import _GEMM_SIZE, measure_gemm_tflops
...
class TestIssue444BestRepeatSelection:
    ...
    def test_gemm_ceiling_defaults_samples_to_empty_tuple(self) -> None:
        from souplite.utils.layer_stream_runtime import GemmCeiling
    ...
    def test_selection_picks_maximum_repeat_not_first_or_last(
        ...
        from souplite.utils import layer_stream
...
class TestDiskKindDetection:
    ...
    def test_reports_one_of_the_known_kinds(self):
        from souplite.utils.layer_stream import DISK_KINDS, detect_disk_kind
    ...
    def test_result_is_cached_per_volume(self, monkeypatch):
        ...
        loaded CI runner."""
        import souplite.utils.layer_stream as ls
    ...
    def test_windows_bus_type_beats_media_type(self):
        ...
        from souplite.utils.layer_stream import _windows_kind
...
class TestDoctorReportsTheDiskKind:
    ...
    def test_each_media_type_gets_its_own_verdict(self, monkeypatch, kind, expected):
        ...
        monkeypatch.setattr(
            "souplite.utils.layer_stream.detect_disk_kind", lambda *_a, **_k: kind
        )
    ...
    def test_the_probe_is_opt_in_so_doctor_stays_fast(self, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr(
            "souplite.utils.layer_stream.detect_disk_kind",
    ...
    def test_a_failing_probe_degrades_to_unknown_instead_of_crashing(
        ...
        monkeypatch.setattr("souplite.utils.layer_stream.detect_disk_kind", _boom)
        result = CliRunner().invoke(app, ["doctor", "--disk"])
...
class TestPhysicalRamBudget:
    ...
    def test_free_ram_fallback_note_names_resident_extras(self):
        from souplite.utils.layer_stream import TIER_DISK, build_stream_plan
    ...
    def test_forced_ram_refuses_when_physical_ram_budget_is_exceeded(self):
        from souplite.trainer.stream_setup import _validate_qwen4_ngram_ram_fit
    ...
    def test_total_ram_bytes_reports_psutil_total(self, monkeypatch):
        ...
        from souplite.utils.layer_stream import total_ram_bytes
...
class TestDiskTier:
    ...
    def test_a_missing_shard_closes_the_handles_already_opened(self, tmp_path):
        ...
        from souplite.utils.layer_stream_runtime import DiskSource, RamSource
...
class TestAutoTierFallback:
    ...
    def _run(
        ...
        monkeypatch.setattr(
            "souplite.utils.spectrum_scan.resolve_model_weights", lambda *_a, **_k: weights
        )
        monkeypatch.setattr(
            "souplite.utils.layer_stream.free_ram_bytes", lambda: free_ram
        )
        ...
        import souplite.utils.layer_stream as _ls
    ...
    def test_stream_pin_threads_require_pin_into_the_runtime_and_announces_cpu(
        ...
        import souplite.trainer.stream_setup as ss
    ...
    def test_ram_refusal_does_not_depend_on_the_disk_kind(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("souplite.utils.layer_stream.detect_disk_kind", _probe)
        with pytest.raises(ValueError, match="stream_source='ram'"):
            self._run(
    ...
    def test_the_measured_probe_actually_runs_from_setup(self, tmp_path, monkeypatch):
        ...
        def _fake_probe(model, *, rows, seq_len, vocab_size, device):
            from souplite.utils.layer_stream_runtime import StepPeak
        ...
        monkeypatch.setattr(
            "souplite.utils.layer_stream_runtime.measure_step_peak_bytes", _fake_probe
        )
        monkeypatch.setattr(
    ...
    def test_a_measured_miss_stops_setup_from_completing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "souplite.utils.layer_stream_runtime.measure_step_peak_bytes",
...
class TestAccumulationAdvice:
    ...
    def test_advises_raising_batch_when_accumulating_at_batch_one(self):
        from souplite.utils.layer_stream import accumulation_advice
