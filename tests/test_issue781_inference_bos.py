...
def _via_chat(tok, messages):
    from souplite.commands.chat import _generate
...
def _via_diff(tok, messages):
    from souplite.commands.diff import _generate
...
class TestDataGenerateLocalProvider:
    ...
    def _sent(self, monkeypatch, tok):
        transformers = pytest.importorskip("transformers")
        from souplite.commands import generate
        ...
        monkeypatch.setattr(
            "souplite.utils.trust_remote.model_requires_trust_remote_code", lambda *a, **k: False
        )
        monkeypatch.setattr(
            "souplite.utils.trust_remote.resolve_trust_remote_code", lambda *a, **k: False
        )
