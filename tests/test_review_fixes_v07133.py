...
class TestVllmTrustRemoteCode:
    ...
    def test_serve_vllm_passes_resolved_trust(self, monkeypatch):
        ...
        import souplite.commands.serve as serve
        ...
        monkeypatch.setattr(
            "souplite.utils.vllm.create_vllm_engine", _fake_engine
        )
        monkeypatch.setattr(
            "souplite.utils.vllm.create_vllm_app",
            lambda **kw: object(),
        )
        serve._serve_vllm(
            model_path=Path("m"),
...
class TestVisionImageContainment:
    def test_absolute_traversal_dropped(self, tmp_path):
        from souplite.data.loader import _validate_vision_images
...
class TestAdapterScope:
    def _run(self, requested, active, names):
        ...
        from souplite.commands.serve import _adapter_scope
...
def _force_two_gpus(monkeypatch):
    ...
    from souplite.utils import launcher as launcher_mod
    from souplite.utils import topology as topo_mod
...
class TestDataInspectEscape:
    def test_bracket_slash_does_not_crash(self, tmp_path, monkeypatch):
        from souplite.commands.data import app as data_app
...
class TestAsrInferMetricGuard:
    def test_oversized_reference_does_not_crash(self, tmp_path, monkeypatch):
        import souplite.commands.infer as infer
...
class TestRunsEscape:
    def _install_fake_tracker(self, monkeypatch, run):
        ...
        monkeypatch.setattr(
            "souplite.experiment.tracker.ExperimentTracker", _FakeTracker
        )
    ...
    def test_list_does_not_crash(self, monkeypatch):
        from souplite.commands.runs import app as runs_app
...
class TestConstantTimeToken:
    def test_wrong_token_401_right_token_ok(self):
        ...
        from souplite.ui.app import create_app, set_auth_token
