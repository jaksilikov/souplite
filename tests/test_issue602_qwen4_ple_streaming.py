...
def test_sparse_reader_preserves_supported_dtypes(tmp_path, torch_dtype, safe_dtype):
    ...
    from souplite.utils.qwen4_ple import SafeTensorRowReader
...
def test_ple_header_rejects_shape_range_mismatch_and_past_eof():
    from souplite.utils.qwen4_ple import _tensor_rows_from_header
...
def test_oq_affine_decoder_matches_mlx_vectors(bits, words, scale, bias, expected):
    ...
    from souplite.utils.oq_affine import AffineQuantSpec, dequantize_affine
...
def test_oq_ple_reader_dequantizes_only_selected_read_only_rows(tmp_path):
    ...
    from souplite.utils.layer_shard import OQExternalTensorPart, OQExternalTensorSpec
    from souplite.utils.qwen4_ple import OQShardedSafeTensorRowReader
...
def test_qwen4_streaming_refuses_unsupported_task_by_name():
    from souplite.trainer.stream_setup import _validate_qwen4_streaming_mode
...
def test_qwen4_ple_disk_streaming_refuses_non_ssd_by_name():
    from souplite.trainer.stream_setup import _validate_qwen4_ngram_disk
...
def test_qwen4_stream_ngram_source_warns_when_checkpoint_has_no_ple():
    from souplite.trainer.stream_setup import _warn_if_ngram_source_unused
...
def test_external_ple_descriptor_reapplies_a_production_sized_element_cap():
    from souplite.utils.layer_shard import ExternalTensorPart, ExternalTensorSpec
...
def test_qwen4_ngram_policy_covers_oq_ram_and_auto_defaults():
    from souplite.trainer.stream_setup import _resolve_qwen4_ngram_source
...
def _drive_qwen4_streaming_setup(tmp_path, monkeypatch, resolve_weights=None):
    ...
    import souplite.trainer.stream_setup as stream_setup
    ...
    monkeypatch.setattr(
        "souplite.utils.layer_stream.stream_arch_of", lambda *_a, **_k: "qwen4_exp"
    )
    monkeypatch.setattr(
        "souplite.utils.layer_shard.resolve_shard_dir",
        lambda *_a, **_k: str(tmp_path / "shards"),
    )
    monkeypatch.setattr(
        "souplite.utils.layer_shard.shard_checkpoint", lambda *_a, **_k: index
    )
    monkeypatch.setattr(
        "souplite.utils.layer_shard.source_weight_bytes", lambda *_a, **_k: 1_024
    )
    monkeypatch.setattr(
        "souplite.utils.spectrum_scan.resolve_model_weights",
        resolve_weights
    ...
    monkeypatch.setattr(
        "souplite.utils.layer_stream.total_ram_bytes", lambda: harness_total_ram
    )
    monkeypatch.setattr(
        "souplite.utils.layer_stream_runtime.RamSource.layer_specs_from_shards",
        lambda *_a, **_k: layer_specs,
    )
    monkeypatch.setattr(
        "souplite.utils.layer_stream_runtime.extras_resident_bytes",
        lambda *_a, **_k: harness_resident_ram,
    )
    monkeypatch.setattr(
        "souplite.utils.layer_stream_runtime.large_layer_store_bytes",
        lambda *_a, **_k: 0,
    )
    monkeypatch.setattr(
        "souplite.utils.layer_stream_runtime.large_layer_buffer_bytes",
        lambda *_a, **_k: 0,
    )
    monkeypatch.setattr(
        "souplite.utils.layer_stream.resolve_disk_kind",
    ...
    monkeypatch.setattr(
        "souplite.utils.layer_stream_runtime.build_streamed_model",
    ...
    monkeypatch.setattr("souplite.utils.layer_stream.build_stream_plan", _plan)
...
def test_planner_fingerprints_config_json_for_qwen4_but_not_for_llama(
    ...
    import souplite.utils.layer_shard as layer_shard
    from souplite.utils.layer_shard import (
...
def test_disk_embedding_returns_rows_on_registered_weight_device():
    ...
    from souplite.utils.qwen4_ple import _disk_embedding
...
def test_external_tensor_bytes_reports_oq_packed_storage():
    ...
    from souplite.utils.qwen4_ple import external_tensor_bytes
...
def test_qwen4_oq_torch_floor_matches_project_and_doctor():
    ...
    from souplite.commands.doctor import EXTRA_GROUPS
