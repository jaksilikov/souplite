...
class TestResolvePreferenceLoss:
    def test_resolve_preference_task_returns_loss(self):
        from souplite.trainer.preference import resolve_preference_loss
...
class TestPreferenceTrainerWrapper:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("ipo", "souplite.trainer.ipo.IPOTrainerWrapper"),
            ("bco", "souplite.trainer.bco.BCOTrainerWrapper"),
        ],
    )
    ...
    def test_setup_does_not_mutate_caller_cfg(self):
        ...
        with mock_patch("souplite.trainer.dpo.DPOTrainerWrapper") as mock_inner:
...
class TestPreferenceTrainRouting:
    def test_sweep_routes_to_preference_wrapper(self):
        ...
        with mock_patch(
            "souplite.data.loader.load_dataset", return_value=fake_dataset,
        ), mock_patch(
            "souplite.utils.gpu.detect_device", return_value=("cpu", "CPU"),
        ), mock_patch(
            "souplite.utils.gpu.get_gpu_info", return_value=fake_gpu_info,
        ), mock_patch(
            "souplite.experiment.tracker.ExperimentTracker",
        ) as mock_tracker_cls, mock_patch(
            "souplite.monitoring.display.TrainingDisplay",
        ), mock_patch(
            "souplite.trainer.preference.PreferenceTrainerWrapper.setup",
        ), mock_patch(
            "souplite.trainer.preference.PreferenceTrainerWrapper.train",
