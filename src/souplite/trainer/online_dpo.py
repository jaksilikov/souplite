...
- **trl <1** — the trainer and judge may live under ``trl.experimental``; the judge is a
  ...
  :func:`souplite.eval.judge.make_soup_pairwise_judge`); a reward model is
...
- **trl 1.x** — pairwise judges were removed; ``OnlineDPOTrainer`` moved to
  ...
  (:func:`souplite.eval.judge.make_judge_reward_func`) — the same pointwise judge
...
def _trl_accepts(param: str) -> bool:
    ...
    ``pip install "souplite[train]"`` resolves — and the wrapper passed a keyword
...
class OnlineDPOTrainerWrapper:
    ...
    def _setup_transformers(self, cfg: SoupConfig, tcfg) -> None:
        ...
        from souplite.utils.peft_wiring import apply_pre_lora_patches
    ...
    def _build_judge_or_reward(self, tcfg) -> dict:
        ...
        if tcfg.online_dpo_judge:
            ...
            from souplite.eval.judge import JudgeEvaluator, validate_judge_api_base
