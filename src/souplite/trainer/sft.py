...
from souplite.utils.gpu import (
    bf16_fp16_flags,
    estimate_batch_size,
...
class SFTTrainerWrapper(StreamingSetupMixin):
    ...
    def _build_rewind_trainer(
        ...
        *,
        ...
        """
        from souplite.trainer.stream_setup import _distributed_launch
        ...
        from souplite.monitoring.rewind_log import RewindLog, dataset_fingerprint
    ...
    def setup(self, dataset: dict):
        ...
        if batch_size == "auto":
            from souplite.utils.batch_probe import pick_batch_size
            from souplite.utils.gpu import get_gpu_info
            ...
            from souplite.utils.batch_probe import make_cuda_probe_fn
        ...
        if tcfg.curriculum:
            from souplite.utils.curriculum import sort_by_length
        ...
        from souplite.utils.layer_stream import should_enable_hf_gradient_checkpointing
        ...
        if hf_grad_ckpt:
            from souplite.utils.gpu import get_gpu_info
            from souplite.utils.gradient_ckpt import (
        ...
        if tcfg.use_galore:
            from souplite.utils.galore import get_galore_optimizer_and_params
        ...
        if self._is_raft:
            ...
            from souplite.trainer.raft import (
                RaftDataCollator,
            ...
            from souplite.utils.multipack_sampler import (
                validate_multipack_architecture,
            )
            from souplite.utils.multipack_trainer import (
        ...
        from souplite.utils.peft_wiring import attach_compile_prefix_callback

        attach_compile_prefix_callback(self.trainer, tcfg, self._output_dir, console)
        if self._quest_metadata is not None:
            from souplite.utils.quest import QuestMetadataCallback
    ...
    def _prepare_raft_dataset(self, dataset: dict, cfg, tcfg):
        ...
        (:func:`souplite.utils.raft.build_raft_prompt`) then tokenised with the
        prompt span masked. When ``citation_faithful`` is set, citation spans
        in the answer get a boosted ``loss_weights`` entry.
        """
        from datasets import Dataset

        from souplite.utils.raft import build_raft_prompt, tokenize_raft_example
    ...
    def _prepare_raft_raw_dataset(self, dataset: dict, cfg, tcfg):
        ...
        :class:`~souplite.trainer.raft.RaftEpochShuffleCollator` re-composes +
    ...
    def _resolve_mixed_precision(self, tcfg, base_model: str) -> tuple[bool, bool]:
        ...
        *"Your setup doesn't support bf16/gpu. You need Ampere+ GPU with
        ...
        from souplite.utils.mixed_precision import pick_mixed_precision
    ...
    def _setup_transformers(self, cfg, tcfg):
        ...
        from souplite.utils.moe import detect_moe_model
        ...
        if tcfg.use_liger:
            from souplite.utils.liger import apply_liger_kernel
        ...
        if tcfg.use_cut_ce:
            from souplite.utils.cut_ce import NO_MATCHING_ARCHITECTURE_MESSAGE, apply_cut_ce
        ...
        # Quantization (v0.38.0 Quant Menu — see souplite.utils.quant_menu)
        from souplite.utils.quant_menu import build_quantization_config_for_loader
        ...
        from souplite.utils.low_ram import force_garbage_collection
        ...
        if tcfg.use_flash_attn:
            from souplite.utils.flash_attn import get_attn_implementation
        ...
        if tcfg.rope_scaling_type:
            ...
            from souplite.utils.long_context import apply_long_context_config
        ...
        from souplite.utils.data_pipeline import apply_vocab_expansion
        ...
        if tcfg.freeze_layers is not None or tcfg.freeze_ratio is not None:
            from souplite.utils.freeze import freeze_model_layers
        ...
        if tcfg.unfrozen_parameters:
            from souplite.utils.freeze import apply_unfrozen_parameters
            ...
            from souplite.utils.peft_wiring import apply_lisa_setup
            ...
            from souplite.utils.mod import apply_mod_if_configured
    ...
    def _apply_quantization_aware(self, tcfg) -> None:
        ...
        if tcfg.quantization_aware and tcfg.quantization_aware != "fp8":
            from souplite.utils.qat import prepare_model_for_qat
    ...
    def _setup_unsloth(self, cfg, tcfg):
        ...
        from souplite.utils.unsloth import load_model_and_tokenizer
    ...
    def _prepare_audio_dataset(self, dataset: dict):
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install \"souplite[audio]\""
            )
    ...
    def train(
        ...
        from souplite.utils.ebft_gdpo import attach_ebft_compute_loss
        ...
        from souplite.utils.activation_offload import offload_context
        from souplite.utils.paths import is_under_cwd

        tcfg = self.config.training
        if self._quest_metadata is not None:
            from souplite.utils.quest import validate_resume_metadata, write_metadata
        ...
        with self._training_context(
            ...
            if getattr(tcfg, "use_longlora", False) and self.config.backend == "transformers":
                from souplite.utils.longlora import apply_longlora_forward_override
        ...
        if self._quest_metadata is not None:
            from souplite.utils.quest import write_metadata
        ...
        if getattr(self.config.training, "use_fsdp2_compile", False) and getattr(
            ...
            from souplite.utils.peft_wiring import strip_compile_prefix
