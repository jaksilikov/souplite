...
Bridges :mod:`souplite.plugins` registered hooks into the HF Trainer
...
def _collect_active_hooks() -> list[tuple[str, dict[str, Any]]]:
    ...
    Lazy-imports :mod:`souplite.plugins` so the callback module stays
    cheap to import in CI / non-training contexts.
    """
    from souplite.plugins import discover_hooks, list_plugins, load_plugins
