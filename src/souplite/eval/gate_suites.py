...
from souplite.eval.forgetting import MINI_BENCHMARKS, ForgettingDetector
...
def _load_gate_fixture(filename: str) -> Tuple[dict, ...]:
    ...
    try:
        ref = files("souplite") / "data" / "_fixtures" / "gate" / filename
...
def _score_refusal_suite(items: Tuple[dict, ...], gen: GeneratorFn) -> float:
    ...
    """
    from souplite.utils.diagnose.refusal import looks_like_refusal
