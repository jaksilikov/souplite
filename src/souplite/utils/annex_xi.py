...
_Generated by souplite {_md_escape(data.soup_version)} at {_md_escape(data.created_at)}._
...
def render_annex_pdf(data: AnnexXIData, section: str) -> bytes:
    ...
    Lazy-imports ``reportlab`` (install via ``pip install souplite[pdf]``).
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "`pip install souplite[pdf]` (or `pip install reportlab`)."
        ) from exc
