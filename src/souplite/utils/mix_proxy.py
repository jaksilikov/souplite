...
recipe, invokes ``python -m souplite.cli train --config <tmp> --yes`` via a
...
- Per-candidate temp dir lives under ``tempfile.gettempdir()``; cleaned up
  ...
  policy).
- Subprocess invocation uses ``sys.executable + ['-m', 'souplite.cli', ...]``
...
def _validate_path_under_cwd(name: str, raw: object) -> str:
    ...
    from souplite.utils.paths import is_under_cwd  # noqa: PLC0415
...
def _read_final_eval_loss(
    ...
    """
    from souplite.experiment.tracker import ExperimentTracker  # noqa: PLC0415
