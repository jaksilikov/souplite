...
'souplite[compile]'``) so the command works without them installed.
...
The CLI command lives in ``souplite/commands/compile_cmd.py`` (named
...
_INSTALL_HINT = (
    "Run: pip install \"souplite[compile]\"  (installs dspy-ai / textgrad / gepa)"
)
...
def run_compile(plan: CompilePlan) -> CompileResult:
    ...
    friendly ``ImportError`` (naming ``pip install souplite[compile]``) when it
