...
- ``llm_text`` + ``judge`` route through the v0.20.0 provider catalog via
  :func:`souplite.utils.data_forge.make_judge_provider_fn` (so the same
  SSRF / env-key surface applies).
- ``code`` reuses the v0.25.0 RLVR sandbox via
  :func:`souplite.trainer.rewards._run_code_sandbox`.
"""
...
_LOG = logging.getLogger("souplite.utils.recipe_run")
...
def run_recipe(
    ...
    *,
    ...
    from souplite.utils.recipe_dag import RecipeDAG
