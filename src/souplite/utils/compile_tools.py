...
``ImportError`` (``pip install "souplite[compile]"``).
...
_INSTALL_HINT = (
    "Run: pip install \"souplite[compile]\"  (installs textgrad / gepa)"
)
...
def run_tool_compile(plan: ToolCompilePlan) -> int:
    ...
    from souplite.utils.agent_forge import load_spec_file, parse_spec
