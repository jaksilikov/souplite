...
from souplite.utils.ingest_sources import _MAX_INGEST_LINES
...
_LOG = logging.getLogger("soup.ingest.pull")
...
def _is_loopback(host: str) -> bool:
    from souplite.utils.net_guard import LOOPBACK_HOSTS, parse_ip_literal
...
def _is_private(url: str) -> bool:
    from souplite.utils.net_guard import is_private_or_link_local
...
def _urllib_transport(url: str, credentials: LangfuseCredentials, timeout: float) -> HttpResponse:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": f"souplite/{__version__} (ingest --pull)",
        },
        method="GET",
    )
    try:
