...
from souplite.utils.async_disk_source import DEFAULT_STREAM_READ_AHEAD
...
def validate_quant_shape(key: str, spec: Any, shard_spec: Mapping[str, Any]) -> None:
    ...
    """
    from souplite.utils.layer_shard import ABSMAX_SUFFIX, NESTED_ABSMAX_SUFFIX
...
def large_layer_specs(shard_dir: str, index: Any) -> Dict[str, Tuple[Tuple[int, ...], str]]:
    ...
    from souplite.utils.layer_shard import large_shard_path, large_weight_role
...
def measure_step_peak_bytes(
    ...
    *,
    ...
    :func:`~souplite.utils.layer_stream.measure_logits_loss_bytes_per_element`
    already keeps.
    """
    if not str(device).startswith("cuda"):
        return None
...
def build_meta_skeleton(
    ...
    *,
    ...
    from souplite.utils.layer_shard import QUANT_NF4, SUPPORTED_STREAM_QUANTS
...
def build_streamed_model(
    *,
    ...
    if external_tensors:
        ...
        from souplite.utils.qwen4_ple import install_qwen4_ple_embeddings
