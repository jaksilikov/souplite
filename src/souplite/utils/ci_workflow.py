  ...
  soup-gate:
    ...
    steps:
      ...
      - name: Run expectations suite
        run: |
          set +e
          soup expect {data} {suite}
          rc=$?
          set -e
          if [ "$rc" -eq 2 ]; then
            echo "[soup] expectations suite failed: quality regression" >&2
            exit 2
          elif [ "$rc" -ne 0 ]; then
            echo "[soup] expectations suite error: usage or configuration error (exit $rc)" >&2
            exit "$rc"
          fi
