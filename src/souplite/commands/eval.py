...
def benchmark(
    ...
    try:
        ...
        console.print(
            ...
            "Install with: [bold]pip install \"souplite\\[eval]\"[/]"
        )
        raise typer.Exit(1)
...
def aider(
    ...
    from souplite.eval.aider_polyglot import (
        AiderEvalError,
...
def custom(
    ...
    from souplite.eval.custom import load_eval_tasks
    ...
    results_list = []
    from souplite.eval.custom import _create_default_generator, score_task
    ...
    from souplite.eval.custom import EvalResults
    ...
    if output or attach_to_registry:
        from souplite.registry.attach import attach_artifact, write_eval_json
...
def judge(
    ...
    from souplite.eval.judge import JudgeResults
...
def compare(
    ...
    from souplite.eval.leaderboard import compare_runs
    from souplite.experiment.tracker import ExperimentTracker
...
def leaderboard(
    ...
    from souplite.eval.leaderboard import (
...
def human(
    ...
    from souplite.eval.human import (
        HumanEvalResults,
        HumanJudgment,
        load_prompts,
        save_results,
    ...
    from souplite.eval.custom import _create_default_generator
...
def _save_custom_results(
    ...
    from souplite.eval.custom import TASK_NAME
    from souplite.experiment.tracker import ExperimentTracker
...
def gate_cmd(
    ...
    from souplite.eval.gate import (
        load_suite,
        resolve_baseline,
        run_gate,
        write_baseline_file,
    ...
    if model is None:
        ...
        from souplite.eval.quant_check import make_model_generator
...
def quant_check_cmd(
    ...
    """
    from souplite.eval.quant_check import (
        ensure_format,
        is_under_cwd,
        render,
        resolve_model_ref,
...
from souplite.commands._eval_v0550 import register as _register_v0550  # noqa: E402

_register_v0550(app, console)

# Register v0.61.0 subcommands (eval unlearning)
from souplite.commands._eval_v0610 import register as _register_v0610  # noqa: E402

_register_v0610(app, console)

# Register v0.65.0 subcommands (eval behavior / capability / checklist / irt-subset)
from souplite.commands._eval_v0650 import register as _register_v0650  # noqa: E402

_register_v0650(app, console)

# Register v0.71.10 subcommand (eval citation — #202)
from souplite.commands._eval_v07110 import register as _register_v07110  # noqa: E402
