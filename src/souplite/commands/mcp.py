...
def reconcile(
    ...
    from souplite.experiment.tracker import ActiveLaunchingRunError, ExperimentTracker
...
def serve(
    ...
    try:
        from souplite.mcp_server.server import run_stdio_server
    except ImportError:
        # NB: escape the '[' in 'souplite[mcp]' so Rich prints it literally
        # instead of parsing '[mcp]' as a (dropped) markup tag.
        console.print(
            "[red]The MCP server needs the 'mcp' SDK.[/] "
            "Install it with: [bold]pip install \"souplite\\[mcp]\"[/]"
        )
    ...
    try:
        from souplite.mcp_server.server import (
        ...
        console.print(
            ...
            "Upgrade with: [bold]pip install -U \"souplite\\[mcp]\"[/] "
