...
if TYPE_CHECKING:  # pydantic models — import for typing only (no eager cost)
    from souplite.config.schema import ShipConfig, SoupConfig
...
def _config_sha_of(cfg: "SoupConfig") -> str:
    ...
    """
    from souplite.registry.hashing import hash_config
...
def _compute_provenance(cfg: "SoupConfig") -> Dict[str, object]:
    ...
    """
    from souplite.eval.gate import current_baseline_stamp
...
def _resolve_generators(
    ...
    from souplite.eval.gate_suites import BEHAVIOURAL_MAX_NEW_TOKENS
    from souplite.utils import live_eval
...
def _leg1_metric(
    ...
    from souplite.eval.custom import load_eval_tasks, run_eval
...
def _build_pairwise_scorer(
    ...
    from souplite.eval.custom import load_eval_tasks
    from souplite.eval.gate import _parse_judge_url
    from souplite.eval.judge import JudgeEvaluator, pairwise_winrate
...
def _lm_eval_leg2(
    ...
    from souplite.commands import eval as eval_cmd
...
def _measure_noise_floor(
    ...
    *,
    ...
    """
    from souplite.eval.gate_suites import is_bundled_suite, score_bundled_suite
...
def _push_pr_comment(verdict: ShipVerdict, target: str) -> None:
    ...
    """
    from souplite.utils import adapter_pr
    from souplite.utils.ship_verdict import render_ship_pr_markdown
