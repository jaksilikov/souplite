...
if TYPE_CHECKING:
    from souplite.utils.deploy_autopilot import DeployProfile
...
_GRADIO_APP_PY = '''"""SoupLite-generated Gradio Chat Space."""
...
chat = gr.ChatInterface(respond, title="SoupLite Fine-tuned Chat")
chat.launch()
'''
...
Generated with [SoupLite](https://github.com/MuhtarJaksilikov/Soup) — model: `{MODEL_REPO}`
"""


_STREAMLIT_APP_PY = '''"""SoupLite-generated Streamlit Chat Space."""
...
def ollama(
    ...
    from souplite.utils.ollama import (
        OLLAMA_TEMPLATES,
        create_modelfile,
        deploy_to_ollama,
        detect_ollama,
...
def hf_space(
    ...
    try:
        if template_dir is not None:
            from souplite.utils.hf_space import (
                detect_space_sdk,
    ...
    try:
        ...
        for in_repo_name, content in files.items():
            api.upload_file(
                ...
                repo_id=space,
                repo_type="space",
                commit_message=f"SoupLite: add {in_repo_name}",
            )
...
def autopilot(
    ...
    """
    from souplite.utils.deploy_autopilot import (
        get_profile,
        list_profiles,
        write_deploy_script,
        write_recipe,
...
def _auto_detect_template() -> Optional[str]:
    ...
    from souplite.utils.ollama import infer_chat_template
