...
from souplite.utils.gpu import detect_device, get_gpu_info, resolve_quantization

if TYPE_CHECKING:  # pragma: no cover - type hints only, no runtime import
    from souplite.config.schema import SoupConfig
    from souplite.utils.energy import EnergyMeasurement
...
def _build_hardware_fit_input(cfg):
    ...
    """
    from souplite.trainer.sft import is_full_finetune
    from souplite.utils.gpu import model_size_from_name
    from souplite.utils.hardware_fit import HardwareFitInput
...
def _hardware_fit_preflight(cfg, gpu_info, *, allow_oom_attempt: bool) -> None:
    ...
    if inp is None:
        return
    from souplite.utils.hardware_fit import VRAM_SAFETY_MARGIN, decide_hardware_fit
...
def train(
    ...
    track_energy: bool = typer.Option(
        ...
        help=(
            ...
            "(offline; requires `pip install souplite\\[carbon]`). Feeds the "
    ...
    if find_lr:
        from souplite.utils.lr_finder import (
            compute_lr_schedule,
    ...
    if getattr(cfg.training, "ra_dit_stage", None) == "generator":
        from souplite.utils.ra_dit_run import autolink_generator_retriever
    ...
    if cloud:
        from souplite import __version__ as _soup_version
        ...
        if cloud == "runpod":
            ...
            from souplite.cloud import modal as cloud_mod
        elif cloud == "lambda":
            from souplite.cloud import lambda_labs as cloud_mod
        else:
            console.print(
    ...
    if push_as:
        from souplite.utils.hf import validate_repo_id
    ...
    if gate:
        from souplite.config.schema import EvalGateConfig
        from souplite.eval.gate import load_suite
    ...
    if hf_resume and push_as and resume_from is None:
        from souplite.monitoring.hf_push import prepare_hf_resume
        from souplite.utils.hf import resolve_endpoint, resolve_token
    ...
    if wandb:
        try:
            ...
            console.print(
                ...
                "Run: [bold]pip install \"souplite\\[wandb]\"[/]"
            )
            raise typer.Exit(1)
    ...
    fsdp_kwargs = None
    if fsdp:
        from souplite.utils.fsdp import FSDP_CONFIGS, get_fsdp_training_args
    ...
    from souplite.utils.quant_menu import resolve_fsdp_qlora_quant_storage
    ...
    from souplite.utils.quant_menu import check_quant_distributed_compat
    ...
    num_gpus = None
    if gpus:
        from souplite.utils.topology import detect_topology, resolve_num_gpus
        ...
        if num_gpus is not None and num_gpus < 1:
            ...
            from souplite.utils.launcher import (
    ...
    from souplite.utils.seeding import resolve_training_seed
    ...
    if cfg.training.use_galore:
        from souplite.utils.galore import validate_galore_config
    ...
    if (
        ...
        from souplite.utils.qat import validate_qat_config
    ...
    if fsdp:
        from souplite.utils.fsdp import validate_fsdp_config
    ...
    if cfg.training.use_liger:
        from souplite.utils.liger import validate_liger_config
    ...
    if cfg.training.use_flash_attn:
        from souplite.utils.flash_attn import validate_flash_attn_config
    ...
    if cfg.training.use_ring_attention:
        from souplite.utils.ring_attention import validate_ring_attention_config
    ...
    if cfg.training.rope_scaling_type:
        from souplite.utils.long_context import validate_long_context_config
    ...
    if cfg.backend == "transformers":
        from souplite.utils.unsloth import is_unsloth_available
    ...
    try:
        ...
        from souplite.utils.trackers import resolve_report_to
        ...
        if hub_name != "hf":
            ...
            from souplite.utils.hubs import download_repo
        ...
        if report_to and report_to not in ("none", "wandb", "tensorboard"):
            from souplite.utils.trackers import tracker_missing_dep_message
        ...
        from souplite.trainer.mlx_routing import resolve_trainer
        ...
        if mlx_cls is not None:
            ...
            from souplite.trainer.prm import PRMTrainerWrapper
            ...
            from souplite.trainer.mole_routing import MoleRoutingTrainerWrapper
            ...
            from souplite.trainer.tts import TTSTrainerWrapper
        ...
        if fsdp and cfg.training.quantization == "4bit":
            from souplite.utils.gpu import get_compute_dtype
            from souplite.utils.mixed_precision import align_trainable_dtype_for_fsdp_qlora
        ...
        if push_as:
            from souplite.monitoring.hf_push import build_push_callback
        ...
        if profile_run:
            from souplite.utils.profiling import profile_training
        ...
        if track_energy:
            try:
                from souplite.utils.energy import EnergyTracker
    ...
    try:
        ...
        try:
            from souplite.utils.crash import build_crash_bundle, write_crash_bundle
    ...
    if track_energy:
        if energy_measurement is not None:
            ...
            console.print(
                ...
                "(install `pip install souplite\\[carbon]`)"
            )
    ...
    if energy_out and _should_run_diagnose_gate_on_rank():
        if energy_measurement is not None:
            ...
            console.print(
                ...
                "(set --track-energy + install `pip install souplite\\[carbon]`)"
            )
...
def _write_annex_xi(out_path: str, run_id: str, cfg, *, energy=None) -> None:
    ...
    from souplite.utils.annex_xi import (
        AnnexXIData,
...
def _write_repro_receipt(out_path: str, run_id: str, cfg) -> None:
    ...
    from souplite.utils.repro_receipt import build_repro_receipt, write_repro_receipt
...
def _capture_activations(
    ...
    *,
    ...
    from souplite.utils import live_eval
...
def _write_energy_json(path: str, measurement: "EnergyMeasurement") -> None:
    ...
    from souplite.utils.paths import atomic_write_text
...
def _run_diagnose_gate(
    ...
    from souplite.utils.diagnose.report import FAILURE_MODES, FailureScore
    from souplite.utils.diagnose.runner import build_report
    from souplite.utils.paths import enforce_under_cwd_and_no_symlink
    ...
    from souplite.utils.diagnose.report import classify_score
...
def _resolve_deepspeed(deepspeed: str) -> str:
    ...
    import souplite.utils.deepspeed as ds
...
def _live_lr_sweep_from_config(cfg, schedule: list[float]) -> list[float]:
    ...
    from souplite.utils.lr_finder import run_lr_sweep
