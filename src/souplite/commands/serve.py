...
def _validate_adapter_path(path: str, cwd: Optional[str] = None) -> bool:
    ...
    from souplite.utils.paths import is_under
...
def serve(
    ...
    if steer is not None:
        ...
        from souplite.utils.steering import (
    ...
    if hub and hub != "hf":
        from souplite.utils.hubs import apply_hub_to_cli_model
    ...
    try:
        ...
        console.print(
            ...
            "Install with: [bold]pip install \"souplite\\[serve]\"[/]"
        )
        raise typer.Exit(1)
    ...
    if backend == "mii":
        from souplite.utils.mii import (
            build_mii_app,
            create_mii_pipeline,
            is_mii_available,
        )
    ...
    if backend == "transformers":
        from souplite.utils.vllm import is_vllm_available
        ...
        if is_vllm_available():
            ...
            from souplite.utils.sglang import check_sglang_available
    ...
    if backend == "vllm":
        ...
        if not is_vllm_available():
            console.print(
                ...
                "Install with: [bold]pip install \"souplite\\[serve-fast]\"[/]"
            )
            raise typer.Exit(1)
    ...
    if backend == "sglang":
        ...
        if not check_sglang_available():
            console.print(
                ...
                "Install with: [bold]pip install \"souplite\\[sglang]\"[/]"
            )
            raise typer.Exit(1)
    ...
    if auto_spec and not speculative_model:
        ...
        # ESC/OSC sequences live (mirrors souplite.utils.terminal.for_terminal).
        _safe = for_terminal
    ...
    from souplite.utils.structured_output import validate_mode
    ...
    if trace and trace_endpoint:
        from souplite.utils.tracing import validate_otlp_endpoint
    ...
    if backend == "vllm":
        ...
        if mole is not None:
            ...
            from souplite.utils.mole_routing import load_mole_for_serve
        ...
        if bank is not None:
            ...
            from souplite.utils.vector_bank import (
                apply_bank_to_serve,
                load_bank,
            )
        ...
        from souplite.utils.paths import is_under_cwd
        from souplite.utils.structured_output import build_constraint
        ...
        from souplite.utils.tracing import build_tracer
        ...
        if trace_log is not None:
            from souplite.monitoring.trace_logger import TraceLogWriter
        ...
        if reasoning_parser:
            from souplite.utils.reasoning_parser import validate_parser_name
        ...
        if record_thumbs is not None:
            ...
            from souplite.utils.local_rl import init_local_rl_db, validate_db_path
...
def _serve_vllm(
    ...
    from souplite.utils.vllm import create_vllm_app, create_vllm_engine
...
def _serve_sglang(
    ...
    from souplite.utils.sglang import create_sglang_app, create_sglang_runtime
...
def _generate_response(
    ...
    from souplite.utils.vllm import encode_chat_prompt
...
def _create_app(
    ...
    from souplite.utils.metrics import ServerMetrics
        ...
        with _contextlib.ExitStack() as stack:
            ...
            try:
                try:
                    ...
                    from souplite.utils.structured_output import (
                ...
                if reasoning_parser is not None:
                    from souplite.utils.reasoning_parser import strip_reasoning
    ...
    def anthropic_messages(payload: dict) -> dict:
        from souplite.utils.anthropic_messages import (
            from_anthropic,
    ...
    def tool_python(
        ...
        if len(code) > tool_max_code_len:
            raise HTTPException(status_code=400, detail="Invalid request")
        try:
            from souplite.trainer.rewards import _run_code_sandbox
    ...
    def record_thumb_endpoint(
        ...
        if (
            ...
            raise HTTPException(status_code=400, detail="Invalid request")
        from souplite.utils.local_rl import record_thumb
