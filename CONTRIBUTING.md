...
```bash
ruff check src/souplite/ scripts/ tests/ benchmarks/
```
...
# Auto-fix issues
ruff check --fix src/souplite/ scripts/ tests/
```
...
```bash
pytest tests/ --cov=souplite --cov-report=html
```
...
- **If a maintainer ends up doing the work instead, you get told, in the thread,
  before or when it lands.** This has been broken once ([#313](https://github.com/MuhtarJaksilikov/Soup/issues/313)):
...
- **A claim that goes quiet is released with a comment, not silently.** We check
  ...
  [#280](https://github.com/MuhtarJaksilikov/Soup/issues/280) for the shape.
...
- [ ] `ruff check src/souplite/ scripts/ tests/ benchmarks/` passes
...
3. Add a template YAML under `src/souplite/templates/` + a `manifest.json` entry (see the existing 21 templates)
...
2. Register in `src/souplite/cli.py` with `@app.command()`
...
Look for issues labeled [`good first issue`](https://github.com/MuhtarJaksilikov/Soup/labels/good%20first%20issue) on GitHub. These are beginner-friendly tasks that help you get familiar with the codebase.
...
- **Bug fixes** — check [open issues](https://github.com/MuhtarJaksilikov/Soup/issues) labeled `bug`
...
2. Update version in `pyproject.toml` and `src/souplite/__init__.py`
...
- **Issues:** Report bugs and request features on [GitHub Issues](https://github.com/MuhtarJaksilikov/Soup/issues)
- **Discussions:** Ask questions on [GitHub Discussions](https://github.com/MuhtarJaksilikov/Soup/discussions)
