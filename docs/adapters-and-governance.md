...
VeRA / VB-LoRA bank storage (`souplite.utils.vector_bank`) supports live multi-tenant serving,
...
```bash
# 1. Train and persist the measured energy (needs `pip install souplite[carbon]`):
...
the **`ed25519` backend is live** (`pip install souplite[sign]`):
...
soup train --config soup.yaml --annex-xi annex.pdf           # PDF (pip install souplite[pdf])
...
souplite[carbon]`; without it, `--track-energy` is a graceful no-op.
...
(`pip install souplite[sign]`): it adds a real detached signature over the
