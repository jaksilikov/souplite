...
`none` ([#423](https://github.com/MuhtarJaksilikov/Soup/issues/423)). The
...
([#686](https://github.com/MuhtarJaksilikov/Soup/issues/686)). Before that they
...
```bash
pip install "souplite[modal]"   # only needed for live submit
...
`soup_modal_app.py` builds an image with `souplite[train]` pinned to your running version, writes
...
```python
from souplite.utils.profiling_v0_43 import (
...
```python
from souplite.utils.vscode_setup import write_vscode_launch
...
```python
from souplite.utils.onboarding import render_onboarding_yaml
...
```python
from souplite.utils.hubs import download_repo, upload_repo
...
```
--tracker mlflow requires the 'mlflow' package. Install with: pip install souplite[trackers] (or pip install mlflow)
```

```bash
pip install souplite[trackers]   # mlflow + swanlab + trackio
```
...
Soup discovers bundled modules under `souplite.plugins` and installed Python
distributions that publish the `souplite.plugins` entry-point group. An external
plugin exposes a zero-argument registration function:

```python
from souplite.plugins import register_plugin
...
```toml
[project.entry-points."souplite.plugins"]
...
```python
from souplite.utils.integrations import list_integrations, get_integration
...
```python
from souplite.utils.hardware_fit import HardwareFitInput, decide_hardware_fit
...
- **`soup version` ≠ `pip show souplite`** — multiple Python installs; use a virtualenv.
