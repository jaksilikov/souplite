...
All notable changes to **SoupLite** are documented here.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

Detailed, per-release notes for every published version live on the
[GitHub Releases page](https://github.com/MuhtarJaksilikov/Soup/releases). This
...
- **The README now leads with `pipx`, and names PEP 668 by its error text (#671 by @swalla02 in #673).**
  `pip install souplite`, the first command in the README, fails on Debian 12 and
  ...
  than a library, so the install now leads with `pipx install souplite` and
  `uv tool install souplite`. Both give Soup its own virtualenv and put the
...
- **Breaking: an unknown config key now refuses the load (#627 deadline, in #879).**
  ...
  privately; the loader now uses the shared `souplite.utils.terminal.for_terminal`), and
...
- Fixed `soup doctor` treating the optional `[train]` stack as missing required dependencies: a core-only install now suggests `pip install "souplite[train]"` once (with the CUDA wheel index on NVIDIA GPUs) instead of bare per-package floors, `soup version --full` derives installed extras from distribution metadata, and a missing or incompatible core dependency exits non-zero (#828 by @wangzhengzhuo05 in #854).
...
- **`soup data validate --format` accepted any string and reported every row valid for it
  ...
  checked the value against `souplite.data.formats.VALID_FORMATS` before `validate_and_stats`
...
- **A weekly `dependency drift` job, and a test that asks trl what it still
  ...
  caps it below 5, so `pip install "souplite[train,mlx]"` cannot resolve.
...
- [#572](https://github.com/MuhtarJaksilikov/Soup/pull/572) adds a weight-free
...
- [#575](https://github.com/MuhtarJaksilikov/Soup/pull/575) adds opt-in PEFT
...
- [#576](https://github.com/MuhtarJaksilikov/Soup/pull/576) adds
...
- [#582](https://github.com/MuhtarJaksilikov/Soup/pull/582) adds a ready-made
...
- **Unknown config keys are now reported instead of silently dropped, and v0.75 will reject them (#627 by @Srinivasan8888 in #628).**
  ...
  declared `souplite.__version__` by a test, so the release that crosses the deadline turns a
...
- **Every DeepSpeed-capable trainer now prunes the empty LoRA optimizer group,
  ...
  changing anything: 19 modules under `souplite/trainer/` accept a
  `deepspeed_config` and exactly one called the guard, so 18 tasks still died
  the same way under `--deepspeed` with LoRA.

  Coverage is enforced by a scan over `souplite/trainer/*.py` rather than a
...
- [#564](https://github.com/MuhtarJaksilikov/Soup/pull/564) enables BF16 autocast for
...
- [#568](https://github.com/MuhtarJaksilikov/Soup/pull/568) enables runtime-probed
...
- [#574](https://github.com/MuhtarJaksilikov/Soup/pull/574) corrects the MCP
...
- [#581](https://github.com/MuhtarJaksilikov/Soup/pull/581) ports two v0.73.0
...
- [#588](https://github.com/MuhtarJaksilikov/Soup/pull/588) makes FSDP + BNB
...
- [#631](https://github.com/MuhtarJaksilikov/Soup/pull/631) pins the last
...
- **The OTLP tracing endpoint validator now rejects abbreviated, decimal, hex,
  ...
  walks `src/souplite/` for any function whose name ends in
...
- **The MLX SFT dispatch route no longer imports the Transformers SFT wrapper
  ...
  today, but the standalone `souplite[mlx]` runtime no longer depends on it
...
- **`soup env check` now flags an installed package that violates Soup's own
  declared version bound (#368 by @ousamabenyounes in #421).** `pip install "souplite[serve-fast]"` (vllm)
...
- **bf16 was assumed on every CUDA card, so the entire free GPU tier failed (#385, #387).**
  ...
  **This cannot regress a working setup**: where bf16 is supported the answer is unchanged,
  ...
  `souplite/trainer/` rather than parametrising over a hand-written list — the list is what
...
- **`device_map="auto"` broke every distributed launch, in fifteen places.**
  ...
  `souplite/trainer/` instead of a hand-written list — the list is what hid the nine.
...
`pip install 'souplite[train]'` and resolved to a recent `trl` had
...
(`pip install souplite` without the `[train]` extra) was never broken — it fell
...
runtime (`import souplite.cli` must not put `torch` in `sys.modules`) instead of
...
- `souplite/utils/layer_stream.py` — tier choice, pinned-vs-pageable decision,
  architecture allowlist, VRAM/throughput arithmetic (no torch import).
- `souplite/utils/layer_shard.py` — rewrites an HF checkpoint into one
  safetensors shard per decoder layer, one tensor at a time, so sharding a model
  that does not fit never needs it to fit.
- `souplite/utils/layer_stream_runtime.py` — buffer pool, CPU-RAM weight source,
...
**Every `pip install souplite[extra]` command now works on Windows `cmd.exe`**, and
...
- **Install hints are now quoted so they work in every shell.** Soup printed
  `pip install 'souplite[ui]'` — bash / zsh / PowerShell syntax. `cmd.exe` has no
  ...
  "souplite[extra]"`, which works in cmd, PowerShell, bash, and zsh alike — the
...
- **Eval-gate `type: benchmark` tasks now actually run.** `eval/gate.py` probed
  ...
  ([#315](https://github.com/MuhtarJaksilikov/Soup/pull/315), closes
  [#310](https://github.com/MuhtarJaksilikov/Soup/issues/310))
...
- **The hardware-fit gate refused to train any local checkpoint.** A merged
  ...
  (`M` suffix) fixes.
- **`pip install 'souplite[extra]'` hints printed without the extra.** Rich ate
  the bracket, so every "install the missing dependency" message across 17 sites
  told users to run `pip install 'souplite'` — which succeeds and still leaves
...
- **PRM-guided GRPO scores completions in a single batched forward.**
  ...
  Closes #298
  ([#301](https://github.com/MuhtarJaksilikov/Soup/pull/301) by [@Ekaanksh-dev](https://github.com/Ekaanksh-dev)).
...
- **Bundled rollout environments** — three pure-Python toy environments
  (`souplite.envs.calculator` / `retrieval_qa` / `guess_number`) exposing a
  `rollout(prompts)` entry point so the live `openenv` GRPO rollout path runs
  out-of-the-box: `training.rollout_backend=openenv` +
  `training.rollout_func=souplite.envs.calculator:rollout`. Three ready-made
...
- **MCP server (`soup mcp serve`)** — drive Soup from any Model Context Protocol
  ...
  `[mcp]` extra (`pip install 'souplite[mcp]'`), lazy-imported so the core CLI
...
- The DPO / IPO / KTO / BCO trainers now apply configured vocabulary expansion
  ...
  ([#293](https://github.com/MuhtarJaksilikov/Soup/pull/293) by [@CODING-DARSH](https://github.com/CODING-DARSH)).
- The ORPO / SimPO / GRPO trainers now also apply configured vocabulary
  ...
  Closes #294
  ([#295](https://github.com/MuhtarJaksilikov/Soup/pull/295) by [@CODING-DARSH](https://github.com/CODING-DARSH)).
...
- Harden judge-model URL validation against a hostname prefix bypass
  ...
  ([#288](https://github.com/MuhtarJaksilikov/Soup/pull/288) by [@CODING-DARSH](https://github.com/CODING-DARSH)).
- `SFTTrainerWrapper` now applies configured vocabulary expansion
  ...
  ([#287](https://github.com/MuhtarJaksilikov/Soup/pull/287) by [@CODING-DARSH](https://github.com/CODING-DARSH)).
- Vision and audio SFT paths now apply that same configured vocabulary
  ...
  ([#291](https://github.com/MuhtarJaksilikov/Soup/pull/291) by [@CODING-DARSH](https://github.com/CODING-DARSH)).
...
- Ready-made `qwen2.5-coder-7b-sft` recipe for `Qwen/Qwen2.5-Coder-7B-Instruct`
  (catalog 133 → 134)
  ([#285](https://github.com/MuhtarJaksilikov/Soup/pull/285) by [@Deadpool2000](https://github.com/Deadpool2000)).
...
- Friendlier error messages: the CUDA-OOM hint now also suggests
  ...
  ([#282](https://github.com/MuhtarJaksilikov/Soup/pull/282) by [@Akshaya-reddy18](https://github.com/Akshaya-reddy18)).
...
- Unit-test coverage for the `warmup.py` auto-warmup-steps helper
  ([#274](https://github.com/MuhtarJaksilikov/Soup/pull/274) by [@shatakshi-1404](https://github.com/shatakshi-1404)).
...
- **`soup train --cloud modal`** (closes #16). Render a self-contained Modal.com
  ...
  `[modal]` extra (`pip install 'souplite[modal]'`; only needed for live submit —
...
- **`soup compile` runs DSPy / GEPA / TextGrad prompt-program optimisation** (#225)
  ...
  (`pip install 'souplite[compile]'`) with a friendly `ImportError` naming the
...
- **`soup edit set` (ROME / MEMIT / AlphaEdit) is live** (#194). New
  `souplite/utils/edit_kernels.py` ships covariance-free rank-1 weight-edit
...
- **`soup train --task unlearn` is live (NPO / SimNPO / RMU)** (#193). New
  `souplite/utils/unlearn_kernels.py` (NPO `(2/β)·mean(-logσ(-β(πlp-reflp)))`,
...
- **Probes & SAE — real weights + live downloads** (closes #215, #216, #217,
  #218, #219). A new shared `souplite/utils/probe_kernel.py` provides the
...
- **`soup probe sae-diff <repo> --auto-download`** (#216) — fetch an
  ...
  `souplite.utils.hubs.snapshot_download` (repo-id shape + home/cwd/tmp cache
...
- **Eval live runners** — six probe surfaces that previously emitted heuristic
  ...
  #211, #212, #165). New shared `souplite/utils/live_eval.py` provides the
...
- **`soup diagnose --base-model <id> [--adapter <path>] [--dataset <jsonl>]
  ...
  `souplite.utils.diagnose.live.run_live_diagnose`; falls back to neutral OK or
...
- **Slack/Discord webhooks on four more commands** — `--slack-url` / `--discord-url`
  ...
  moved to a shared `souplite/utils/webhooks.py`.
...
- **Energy & CO2 measurement for training** — `soup train --track-energy` wraps
  ...
  codecarbon is absent (`pip install souplite[carbon]`). `--energy-country`
...
- **ed25519 signing for `soup adapters sign` / `soup attest`** — real detached
  ...
  `[sign]` extra (`pip install souplite[sign]`, pulling `cryptography`).
...
- **Breaking — install split.** The heavy training stack (`torch`,
  ...
  out of the core install into a new `[train]` extra. `pip install souplite` is
  now a light CLI + data-tools install with **no PyTorch**; run
  `pip install 'souplite[train]'` (or `[all]`) to fine-tune. Existing users who
...
- Migrated to a `src/` layout (`src/souplite/`) for cleaner packaging and to
...
- Friendly error mapping: a missing heavy dependency (`torch`, `transformers`,
  ...
  "Training needs the [train] extra. Run: pip install 'souplite[train]'".
...
[Unreleased]: https://github.com/MuhtarJaksilikov/Soup/compare/v0.75.0...HEAD
[0.75.0]: https://github.com/MuhtarJaksilikov/Soup/compare/v0.74.0...v0.75.0
[0.74.0]: https://github.com/MuhtarJaksilikov/Soup/compare/v0.73.3...v0.74.0
[0.73.1]: https://github.com/MuhtarJaksilikov/Soup/compare/v0.73.0...v0.73.1
[0.71.0]: https://github.com/MuhtarJaksilikov/Soup/compare/v0.70.0...v0.71.0
