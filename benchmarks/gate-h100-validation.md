...
```
>>> from souplite.utils.layer_stream import detect_disk_kind
>>> detect_disk_kind()
'hdd'
```
...
```
$ python -m souplite.cli train --config stream05b.yaml --yes      # 8 visible
...
$ CUDA_VISIBLE_DEVICES=6 python -m souplite.cli train --config stream05b.yaml --yes
Layer streaming ready: 24 layers, 0.18 GB pinned RAM store, 2 x 8 MB VRAM buffers
...
```
$ torchrun --nproc_per_node 8 -m souplite.cli train --config b_ds.yaml --deepspeed ...
...
Shipped as `install_dequant_forward` in `src/souplite/utils/layer_stream_runtime.py`,
...
```
accelerate launch --num_processes 4 /root/venv/bin/python -m souplite.cli train ...
...
0.26.2, which is what `pip install "souplite[train]"` resolves today and what this box
...
```bash
until ! pgrep -f "souplite.cli train"; do sleep 10; done
```
...
All of them import `souplite` unmodified: every measurement above the
