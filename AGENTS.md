...
ruff check src/souplite/ tests/  # Lint — must be clean before any commit
```
...
- **Config** is Pydantic v2 in `src/souplite/config/schema.py` — single source of truth.
...
- The Pydantic config schema in [`src/souplite/config/schema.py`](src/souplite/config/schema.py) is the single source of truth for every config field.
